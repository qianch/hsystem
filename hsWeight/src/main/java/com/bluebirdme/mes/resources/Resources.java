...
public class Resources {
    private static final String UI_BASE = "com/bluebirdme/mes/resources/";
    ...
    public JsonObject getBarCodeInfo(String server_addr, String code) {
        try {
            ...
            return jsonParser.parse(doc.text()).getAsJsonObject();
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public JsonObject getTare(String addr, String rollBarcode, String carrierCode) {
        try {
            String infoUrl = addr + "/mobile/weight/tare?rollBarcode=" + (StringUtils.isBlank(rollBarcode) ? "" : rollBarcode) + "&carrierCode=" + (StringUtils.isBlank(carrierCode) ? "" : carrierCode);
