...
public class OutTimeMessageCreator {
    ...
    public void creator() {
        ...
        List<DeliveryPlan> dplist = msgService.findListByMap(DeliveryPlan.class, map);
        map.clear();
        map.put("isComplete", null);
        map.put("closed", null);
        List<SalesOrderDetail> sodList = msgService.findListByMap(SalesOrderDetail.class, map);
        for (DeliveryPlan dp : dplist) {
            // 查找过期的所有出货计划
            if (dp.getDeliveryDate().before(new Date())) {
                //需要自动关闭的超期时间
                Date outDate = new Date(new Date().getTime() - 7 * 24 * 60 * 60 * 1000);
                //关闭所有超过超期时间的计划
                if (dp.getDeliveryDate().before(outDate)) {
                    // 如果出货计划中的订单有已经出货的，不操作
                    map.clear();
                    map.put("deliveryId", dp.getId());
                    List<DeliveryPlanSalesOrders> dpsoList = msgService.findListByMap(DeliveryPlanSalesOrders.class, map);
