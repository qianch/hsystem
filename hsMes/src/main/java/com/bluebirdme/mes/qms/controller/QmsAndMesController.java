...
public class QmsAndMesController extends BaseController {
    private static final Logger log = LoggerFactory.getLogger(QmsAndMesController.class);
    @Resource
    ...
    public String rollBarCodeInfo(String barCode) {
        ...
        if (barCode.startsWith("P") && part != null) {
            try {
                if (part != null) {
                    SalesOrderDetail salesOrderDetail = mobileService.findById(SalesOrderDetail.class, part.getSalesOrderDetailId());
                    if (null != salesOrderDetail) {
                        ret.put("SALESORDERSUBCODE", salesOrderDetail.getSalesOrderSubCode());
                        ret.put("PRODUCTCOUNT", salesOrderDetail.getProductCount());
                        ret.put("PRODUCTMODEL", salesOrderDetail.getProductModel());
                        if (null != salesOrderDetail.getMirrorProcBomVersionId()) {
                            ...
                            ProducePlanDetail detail = mobileService.findById(ProducePlanDetail.class, part.getProducePlanDetailId());
                            Consumer consumer2 = mobileService.findById(Consumer.class, detail.getConsumerId());
                            //客户名称
                            ret.put("CONSUMERCODE", consumer2.getConsumerCode());
                            ret.put("CONSUMERNAME", consumer2.getConsumerName());
                            ret.put("BATCHCODE", part.getBatchCode());
                            param.clear();
                            ...
                            map2.clear();
                            map2.put("salesOrderDetailId", salesOrderDetail.getId());
                            map2.put("procBomId", part.getMirrorProcBomId());
                            map2.put("productId", part.getSalesProductId());
                            ...
                            p = mobileService.findById(FinishedProduct.class, part.getSalesProductId());
                            ret.put(PRODUCT, p);
                            ret.put("GUARDNUMBER", "");
                            ret.put("CLASS", "");
                            ret.put("GUARDNAME", "");
    ...
    public String getDateByCode(String palletCode) {
        Map<String, Object> ret = new HashMap<>();
        try {
    ...
    public String selectOUTOrder(String deliveryCode, String batchCode) throws Exception {
        ...
        if (deliveryCode != null && batchCode != null) {
            ...
            if (outOrders.size() > 0) {
                for (Map<String, Object> order : outOrders) {
                    int tcount = 0;//套数
                    int traycount = 0;//托数
                    Double dweight = 0.00;//一条数据总重量
                    if (order.get("BARCODES") == null) {
                        order.put("TCOUNT", tcount);//套数
                        order.put("TRAYCOUNT", traycount);//托数
                        order.put("DWEIGHT", dweight);//一条数据总重量
                        continue;
                    }
                    String[] barcodes = order.get("BARCODES").toString().split(",");
                    List<String> barcodesLsit = Arrays.asList(barcodes);
                    order.put("BARCODES", barcodesLsit);
                    Map<String, Object> map3 = new HashMap<>();
                    Map<String, Object> map5 = new HashMap<>();
                    List farbicModelfs = new ArrayList();
                    List EmbryoClothNames = new ArrayList();
                    List productProcesName = new ArrayList();
                    if (order.get("PARTID") != null) {
                        TcBomVersionParts tcBomVersionParts = qmsAndMesService.findById(TcBomVersionParts.class, Long.parseLong(order.get("PARTID").toString()));
                        ...
                        map5.put("versionPartsId", Long.parseLong(order.get("PARTID").toString()));
                        map5.put("salesOrderDetailId", Long.parseLong(order.get("SALESORDERDETAILID").toString()));
                        ...
                        if (tcBomVersionPartsMirrors.size() > 0) {
                            ...
                            map5.clear();
                            map5.put("tcProcBomPartsId", Long.parseLong(order.get("PARTID").toString()));
                            ...
                            Map<String, Object> embryoMap = new HashMap<>();
                            embryoMap.put("tcProcBomPartsId", Long.parseLong(order.get("PARTID").toString()));
                    ...
                    if (order.get("PARTID") != null && order.get("SALESORDERID") != null) {
                        Map<String, Object> map6 = new HashMap();
                        map6.put("versionPartsId", Long.valueOf(order.get("PARTID").toString()));
                        map6.put("salesOrderId", Long.valueOf(order.get("SALESORDERID").toString()));
                        ...
                        if (tcBomVersionPartsMirrors.get(0).getTcProcBomVersionPartsName() != null) {
                            order.put("PARTNAME", tcBomVersionPartsMirrors.get(0).getTcProcBomVersionPartsName());
                        }
                    }
                    order.put("FARBICMODELS", farbicModelfs);
                    order.put("EMBRYOClOTHNAMES", EmbryoClothNames);
                    order.put("PRODUCTPROCESSNAME", productProcesName);
                    if (Integer.parseInt(order.get("PRODUCTISTC").toString()) == 2) {
                        List productProcesNameFTC = new ArrayList();
                        productProcesNameFTC.add(order.get("PRODUCTPROCESSNAMEFTC"));
                        order.put("PRODUCTPROCESSNAME", productProcesNameFTC);
                    }
                    //每托重量集合
                    ...
                    order.put("BARCODES", weightList);//每托重量集合
                    order.put("TCOUNT", tcount);//套数
                    order.put("TRAYCOUNT", traycount);//托数
                    order.put("DWEIGHT", dweight);//一条数据总重量
                }
