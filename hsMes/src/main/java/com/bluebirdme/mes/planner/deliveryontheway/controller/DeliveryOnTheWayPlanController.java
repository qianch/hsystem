...
public class DeliveryOnTheWayPlanController extends BaseController {
	...
	public String getProductDeliveryRecord(Filter filter, Page page) throws Exception {
		...
		if (totalWeightrows != null && totalWeightrows.size() > 0) {
			allTotalWeight = new BigDecimal(Double.parseDouble(totalWeightrows.get(0).get("TOTALWEIGHT").toString().replace(",", "")));
			allTotalCount += Integer.parseInt(totalWeightrows.get(0).get("TOTALCOUNT").toString());
		}

		if (allRows != null) {
			for (int i = 0; i < allRows.size(); i++) {
				Map<String, Object> oneRecord = allRows.get(i);
				totalWeight = totalWeight.add(new BigDecimal((Double) oneRecord.get("TOTALWEIGHT")));
			...
			for (int a = 0; a < columnName.length; a++) {
				...
				switch (columnName[a])
				{
					case "序号":
						...
						break;
					case "出库调拨单":
						cell.setCellValue(detail.get("deliveryCode".toUpperCase()).toString());
						break;
					case "仓库名称":
						cell.setCellValue(detail.get("WAREHOUSENAME".toUpperCase())==null?"":detail.get("WAREHOUSENAME".toUpperCase()).toString());
						break;
					case "在途时间":
						cell.setCellValue(detail.getOrDefault("deliveryDate".toUpperCase(),"").toString());
						break;
					case "客户订单号":
					...
					case "部件名称":
						...
						break;
					case "条码编号":
						cell.setCellValue(detail.get("barcode".toUpperCase()).toString());
						break;
					case "重量(kg)":
	...
	public ModelAndView pMove(DeliveryOnTheWayPlan deliveryOnTheWayPlan, String force) throws Exception {
