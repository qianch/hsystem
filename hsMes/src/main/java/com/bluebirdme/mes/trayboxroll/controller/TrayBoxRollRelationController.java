...
public class TrayBoxRollRelationController extends BaseController {
	...
	public String getTrayBoxRollRelation(Filter filter, Page page) throws Exception{
		...
		{
			totalRollWeight=new BigDecimal(Double.parseDouble(rollWeightrows.get(0).get("TOTALROLLWEIGHT").toString().replace(",","")));
		}
		BigDecimal count = new BigDecimal(0);
		for(int i = 0;i<allRows.size();i++){
			Map<String,Object> oneRecord = allRows.get(i);
			count = count.add(new BigDecimal((Double) oneRecord.get("ROLLWEIGHT")));
		}
		Map<String,Object> map = new HashMap<>();
		map.put("ROLLBARCODE","当前页总计：");
		map.put("ROLLWEIGHT",count.setScale(2, BigDecimal.ROUND_HALF_UP)+"kg");
		list.add(map);
		map = new HashMap<>();
		map.put("ROLLBARCODE","当搜索条件总计：");
		map.put("ROLLWEIGHT",totalRollWeight.setScale(2, BigDecimal.ROUND_HALF_UP)+"kg");
		list.add(map);
