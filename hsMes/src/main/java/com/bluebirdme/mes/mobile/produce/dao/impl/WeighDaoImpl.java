...
//		System.out.println(isGroupFirstRoll);
//		System.out.println(index % 10 == 1);
//		System.out.println(index == 2 && rule == 1);
		// 规则不称
		if (rule == 2)
			return false;
		// 除了不称的，首卷必称
...
//
//		long nightEnd=sdf.parse(tomorrow+" 08:00:00").getTime();
		String sql="";
		//当前是早班
	...
	public String hasWaitForWeighRoll(Long planId, String deviceCode) {
		...
		String codes = (String) getSession().createSQLQuery(sql).uniqueResult();
			...
			switch (rule) {
				...
				case 3:
					...
					if(product.getProductRollWeight()==null){
						...
						//定重产品
//				r.setRollWeight(getRollTheoryWeight(barcode));
//				r.setRollAutoWeight(weight);
						if(weight>=product.getMinWeight()&&weight<=product.getMaxWeight()){
							r.setRollWeight(getRollTheoryWeight(barcode));//取理论重量
						}else{//超出偏差取实际值
