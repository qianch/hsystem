...
public class PrinterController extends BaseController {
    ...
    public String getPrinterPartNameInfo(Long weaveId) {
        List<TcBomVersionParts> finalPart = new ArrayList<>();
        List<HashMap<String, Object>> outInfo = new ArrayList<>();
        if (weaveId != null) {
            // 获取编织日计划
            ...
            if (fp.getProductIsTc() == -1) {
                ...
                HashSet<Long> partIdSet = new HashSet<>();
                for (TcBomVersionPartsDetail tvpd : tvpdList) {
                    partIdSet.add(tvpd.getTcProcBomPartsId());
                }
                // bom版本的ID
                HashMap<Long, Long> partIdBomId = new HashMap<>();
                HashSet<Long> bomIdSet = new HashSet<>();
                HashMap<Long, TcBomVersionParts> partsMap = new HashMap<>();
    ...
    public ModelAndView showPrinterPage(Long weaveId, Long cutId, String partName, String departmentName, Integer batchcode) {
        ...
        if (weaveId != null) {
            // 获取编织日计划
            WeavePlan wdp = printerService.findById(WeavePlan.class, weaveId);
            printCount = wdp.getRequirementCount().intValue();
        } else {
            CutPlan cut = printerService.findById(CutPlan.class, cutId);
            printCount = cut.getTotalRollCount();
        }
        return new ModelAndView(printPage, model.addAttribute("weavePlanId", GsonTools.toJson(weaveId)).addAttribute("cutPlanId", cutId).addAttribute("count", printCount).addAttribute("partName", partName).addAttribute("departmentName", departmentName));
    }
    ...
    public ModelAndView showPrinterPageList(String ids, String departmentCode) {
        ...
        printCount = wdp.getRequirementCount().intValue();
        return new ModelAndView(printPageList, model.addAttribute("ids", ids).addAttribute("count", printCount).addAttribute("departmentCode", departmentCode));
    }
    ...
    public ModelAndView showPrinterPage1(Long cutPlanId, Long cutDailyPlanId, String departmentName) {
        ...
        return new ModelAndView(printPage1, model.addAttribute("cutDailyPlanId", cutDailyPlanId).addAttribute("cutPlanId", cpd.getId()).addAttribute("departmentName", departmentName));
    }
