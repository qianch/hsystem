		...
		if (!StringUtils.isEmpty(filterRules)) {
			...
			for (JsonElement obj : array) {
				...
				if(rule.getField().equals("CONSUMERGRADE")){
					if(rule.getValue().toUpperCase().equals("A")){
						filter.set(rule.getField(), "like:1");
					}else if(rule.getValue().toUpperCase().equals("B")){
						filter.set(rule.getField(), "like:2");
					}else if(rule.getValue().toUpperCase().equals("C")){
						filter.set(rule.getField(), "like:3");
					}else{
						filter.set(rule.getField(), "like:-1");
					}
				}else{
