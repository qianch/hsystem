...
public class ProcessServiceImpl extends BaseServiceImpl implements IProcessService {
	...
	@Resource ISalesOrderStockService sosService;
...
//			sod.setProducedRolls((sod.getProducedRolls() == null ? 0 : sod.getProducedRolls()) + 1);
	...
	public void updateBzdb(Long weaveId, Double trayWeight) throws Exception {
		...
		//sosService.addStock(sod.getSalesOrderSubCode(), findById(FinishedProduct.class, sod.getProductId()));
	...
	public void updateCjsc(Long cutId, Double rollWeight) throws Exception {
		...
		//更新总共生产多少套
		//1.根据BOM中各部件需要的数量
		//2. 查询产出登记，对应的套材的各个部件生产的数量，计算出生产出了多少套
		...
		while(it.hasNext()){
			...
			if(producedPartCount.get(entry.getKey())!=null){
				...
				partCount=((BigInteger) entry.getValue()).longValue();
				produceCount=((BigInteger) producedPartCount.get(entry.getKey())).longValue();
		...
		//更新套材生产计划的套数
		//ppd.setProducedCount(min.doubleValue());

		// 订单已生产的数量，就是套数，这里不更新，裁剪打包的时候才更新,只有套材才有裁剪计划
		// 订单已生产数量
		// sod.setProduceCount(sod.getProduceCount() == null ? 0 :
		// ppd.getProducedCount() + rollWeight);
		// 已生产卷数累加
		sod.setProducedRolls((sod.getProducedRolls() == null ? 0 : sod.getProducedRolls()) + 1);
		
		//sod.setProduceCount(min.doubleValue());
...
//		sod.setProducedRolls((sod.getProducedRolls() == null ? 0 : sod.getProducedRolls()) + 1);
	...
	public void updateCjdb(Long cutId, String partName, Double trayWeight) throws Exception {
		...
		//裁剪计划的生产数量，只是重量，没有套数
		//cp.setProducedCount((cp.getProducedCount()==0?0D:cp.getProducedCount())+trayWeight);
...
//		update(cp);
		...
		//更新订单库存
		//sosService.addStock(sod.getSalesOrderSubCode(), findById(FinishedProduct.class, sod.getProductId()),partName);
