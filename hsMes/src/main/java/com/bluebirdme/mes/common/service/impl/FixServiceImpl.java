...
public class FixServiceImpl extends BaseServiceImpl implements IFixService {
	@Resource IFixDao fixDao;
	
	@Resource IBarCodeService barcodeService;
	
	@Resource IBomService bomService;
	
	@Resource ISalesOrderService orderService;
	
	@Resource IWeavePlanService weaveService;
	...
	public void fixRollNO(String[] batchCodes){
		...
		//String batchCodes[]=new String[]{"117070106","117070405","117071206","117071209","117071210"};
	...
	public void fixRollCountInTray(){
		...
		for(TrayBarCode tbc:list){
			...
			update(tbc);
			System.out.println("updated "+(++i)+"/"+list.size());
		}
	...
	public void produceToWeave(Long producePlanDetailId) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException{
		...
		System.err.println("计划数量："+list.size());
			...
			if (ppd.getProductIsTc() == 1 && plan.getWorkshop().contains("裁剪")) {// 如果是套材，到裁剪车间的，那么是非单向布的部件
				...
				for (ProducePlanDetailPartCount ppdpc : ppdpcList) {
					...
					for(TcBomVersionPartsDetail tcvpd:tcvpdList){
						...
						double requirementCount=totalCount*tcvpd.getTcProcBomFabricCount();
						if(tcfp.getProductRollWeight()!=null){
							requirementCount=requirementCount*tcfp.getProductRollWeight();
						}
						//总卷数
...
//						abstractPPD.setPartName(partName);
//						abstractPPD.setPartId(ppd.getPartId());
						
						//abstractPPD.setPlanTotalWeight();
				...
				System.err.println("非套裁,批次号："+ppd.getBatchCode());
				ftcList.add(ppd);
			}else{
				System.out.println("未知"+"其他:"+ppd.getProductIsTc()+"\t车间:"+plan.getWorkshop());
			}
		}
