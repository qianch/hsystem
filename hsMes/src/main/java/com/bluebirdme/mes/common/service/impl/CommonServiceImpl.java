...
public class CommonServiceImpl extends BaseServiceImpl implements ICommonService {
	...
	@Resource ICommonDao commonDao;
	
	@Resource IMessageCreateService msgCreateService;
	...
	public void close(String ids, String type) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		...
		for(String id:_ids){
			if(type.toUpperCase().equals("CUT")){
			...
			case "ORDER":
				{
					...
					String batch=(String) obj.getClass().getMethod("getProductBatchCode", null).invoke(obj, new Object[]{});
//					n.setTitle("订单关闭通知");
//					n.setContent("订单号:"+order+",产品:"+factoryProductName);
...
//					n.setTitle("生产计划关闭通知");
//					n.setContent("计划单号:"+code+",产品:"+factoryProductName+"，批次号:"+batch);
