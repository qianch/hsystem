...
public class DeviceDaoImpl extends BaseDaoImpl implements IDeviceDao {
	...
	public void saveDeviceAndOrder(String dids,String wids){
		...
		WeavePlanDevices wpd=new WeavePlanDevices();
		...
		for(int i=0;i<didArray.length;i++){
			...
			for(int j=0;j<widArray.length;j++){
				
				wp=findById(WeavePlan.class, Long.parseLong(widArray[j]));
				...
				param.clear();
				param.put("deviceId", Long.parseLong(didArray[i]));
				param.put("weavePlanId", Long.parseLong(widArray[j]));
				...
				wpd.setDeviceId(Long.parseLong(didArray[i]));
				wpd.setWeavePlanId(Long.parseLong(widArray[j]));
	...
	public void deleteDeviceAndOrder(String dids,String wids){
		...
		for(int i=0;i<didArray.length;i++){
			...
			for(int j=0;j<widArray.length;j++){
				...
				map.put("deviceId", Long.parseLong(didArray[i]));
				map.put("weavePlanId",Long.parseLong(widArray[j]));
