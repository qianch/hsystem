			...
			for (int j = 0; j < columnName.length; j++) {
				...
				case 13:
					...
					if (data.get("SYNCSTATE") != null) {
						cell.setCellValue(Integer.valueOf(data.get("SYNCSTATE").toString()) == 0 ? "未同步" : "已同步");
					}
					break;
				case 14:
					// 入库类型
					if (data.get("INBANKTYPE") != null) {
						if (Integer.valueOf(data.get("INBANKTYPE").toString()) == 0) {
...
//					}
				...
				case 24:
					...
					if (data.get("MADERATE") != null) {
						cell.setCellValue(Double.valueOf(data.get("MADERATE").toString()));
					}
					break;
		...
		for (Map<String, Object> data : list) {
			...
			for (int j = 0; j < columnName.length; j++) {
				...
				case 19:
					// k3同步状态
					if (data.get("SYNCSTATE") != null) {
						int state = Integer.valueOf(data.get("SYNCSTATE").toString());
						if (state == 0) {
