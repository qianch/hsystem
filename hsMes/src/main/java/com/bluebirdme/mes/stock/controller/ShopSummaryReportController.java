...
public class ShopSummaryReportController extends BaseController {
    private static final Logger logger = LoggerFactory.getLogger(ShopSummaryReportController.class);
    @Resource
    ...
    public String shopStorageCategory(Filter filter, Page page) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            String start = filter.get("start");
            String end = filter.get("end");
            filter.set("end", Objects.requireNonNullElseGet(end, () -> sdf.format(new Date())));
            if (start != null) {
            ...
            if (rows.size() != 0) {
                ...
                for (Map<String, Object> stringObjectMap : r) {
                    weight += (Double) stringObjectMap.get("WEIGHT");
                }
    ...
    public String getProductStock(Filter filter, Page page) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            String start = filter.get("start");
            String end = filter.get("end");
            ...
            if (rows.size() != 0) {
                ...
                Double weight = 0.0;
                for (Map<String, Object> stringObjectMap : r) {
                    String object = stringObjectMap.get("TNUM").toString();
