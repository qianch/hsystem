...
public class ProductsReportController extends BaseController {
    ...
    public String getProductStock(Filter filter, Page page) throws Exception {
        ...
        if (rows.size() != 0) {
            ...
            for (Map<String, Object> stringObjectMap : r) {
                if (stringObjectMap.get("WEIGHT") != null && stringObjectMap.get("WEIGHT") != "") {
                    d += (Double) stringObjectMap.get("WEIGHT");
                }
            }
    ...
    public String productsSundrSummary(Filter filter, Page page) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            ...
            if (rows.size() != 0) {
                ...
                Double aum = 0.0;
                for (Map<String, Object> stringObjectMap : r) {
                    startWight += (Double) stringObjectMap.get("STARTWEIGHT");
                    inWight += (Double) stringObjectMap.get("INWEIGHT");
                    useWight += (Double) stringObjectMap.get("USENUM");
                    outtWight += (Double) stringObjectMap.get("OUTWEIGHT");
                    aum += (Double) stringObjectMap.get("ATUM");
                }
    ...
    public String getFinishFactoryProduct(Filter filter, Page page) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            String start = filter.get("start");
            String end = filter.get("end");
    ...
    public void export(Filter filter) throws Exception {
        ...
        InputStream is = new FileInputStream(PathUtils.getClassPath() + "template/productCategorySummary.xlsx");
        ...
        for (Map<String, Object> data : list) {
            ...
            for (int j = 0; j < 10; j++) {
                ...
                switch (j) {
                    case 0 -> cell.setCellValue(data.get("CATEGORYNAME").toString());//产品大类
                    case 1 -> cell.setCellValue(data.get("CATEGORYCODE").toString());//产品类别代码
                    case 2 -> cell.setCellValue(data.get("STARTWEIGHT").toString());//月初库存数量
                    case 3 -> cell.setCellValue(data.get("INWEIGHT").toString());//当月入库数量
                    case 4 -> cell.setCellValue(data.get("OUTWEIGHTS").toString());//当月领用数量
                    case 5 -> cell.setCellValue(data.get("USENUM").toString());//当月发出数量
                    case 6 -> cell.setCellValue(data.get("ATUM").toString());//月末累计数量
                    default -> {
                    }
                }
            }
        ...
        for (Map<String, Object> data : list) {
            ...
            for (int j = 0; j < 10; j++) {
                ...
                switch (j) {
                    ...
                    case 4 -> cell.setCellValue(data.get("STARTWEIGHT").toString());//月初库存数量
                    case 5 -> cell.setCellValue(data.get("INWEIGHT").toString());//当月入库数量
                    case 6 -> cell.setCellValue(data.get("USENUM").toString());//当月领用数量
                    case 7 ->
                            cell.setCellValue(data.get("OUTWEIGHTS") == null ? "" : data.get("OUTWEIGHTS").toString());//当月发出数量
                    case 8 -> cell.setCellValue(data.get("ATUM").toString());//月末累计数量
                    default -> {
                    }
                }
            }
    ...
    public void export2(Filter filter) throws Exception {
        ...
        List<Map<String, Object>> list = (List<Map<String, Object>>) map.get("rows");
        InputStream is = new FileInputStream(PathUtils.getClassPath() + "template/productSundrSummary.xlsx");
        Workbook wb = new XSSFWorkbook(is);
        Sheet sheet = wb.getSheetAt(0);
        Row row;
        Cell cell;
        ...
        for (Map<String, Object> data : list) {
            ...
            for (int j = 0; j < 12; j++) {
                cell = row.createCell(j);
                switch (j) {
                    case 0 -> cell.setCellValue(data.get("SALESORDERCODE").toString());//订单号
                    case 1 -> cell.setCellValue(data.get("BATCHCODE").toString());//批次号
                    case 2 ->
                    ...
                    case 3 -> {//产品型号
                        ...
                        } else if (Objects.equals(model, "-1")) {
                    ...
                    case 10 -> cell.setCellValue(data.get("ATUM") == null ? "" : data.get("ATUM").toString());//月末累计数量
                    default -> {
                    }
                }
            }
