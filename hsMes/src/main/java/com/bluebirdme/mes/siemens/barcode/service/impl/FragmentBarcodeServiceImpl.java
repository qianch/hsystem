...
public class FragmentBarcodeServiceImpl extends BaseServiceImpl implements IFragmentBarcodeService {
    @Resource
    IFragmentBarcodeDao fragmentBarcodeDao;
    ...
    public void suit(String ctoCode, String part, String fragments, String user, String device) throws Exception {
        ...
        List<PartSuit> list = new ArrayList<>();
        PartSuit suit = null;
        ...
        for (String fg : fragments.split(",")) {
            ...
            list.add(suit);
            sb.append(sb.length() == 0 ? "" : ",").append("'").append(fg).append("'");
        }
        list.add(suit);
        saveList(list);
        ...
        if (Objects.equals(cto.getAssignSuitCount(), cto.getPackedSuitCount())) {
            cto.setIsComplete(1);
        }
        ...
        if (Objects.equals(ct.getSuitCount(), ct.getPackedSuitCount())) {
            ct.setIsComplete(1);
        }
        update(cto);
        update(ct);
