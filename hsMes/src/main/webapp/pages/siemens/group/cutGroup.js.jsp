...
<script>
//添加组别管理
var addUrl=path+"cutGroup/add";
//编辑组别管理
var editUrl=path+"cutGroup/edit";
//删除组别管理
var deleteUrl=path+"cutGroup/delete";

var dialogWidth=300,dialogHeight=200;

//查询
function filter() {
	EasyUI.grid.search("dg","cutGroupSearchForm");
}
...
var add = function() {
	var wid = Dialog.open("添加", dialogWidth, dialogHeight,addUrl, [
		...
		EasyUI.form.submit("cutGroupForm",addUrl, function(data){
			filter();
...
var edit = function(){
	...
	var wid = Dialog.open("编辑", dialogWidth, dialogHeight, editUrl+"?id="+r.ID, [ 
	    EasyUI.window.button("icon-save", "保存", function() {
		EasyUI.form.submit("cutGroupForm", editUrl, function(data) {
			filter();
			Dialog.close(wid);
		})
	}), EasyUI.window.button("icon-cancel", "关闭", function() {
