...
<script>
    ...
    const bomList = path + "siemens/bom/list?showChildren=false";
    const fragmentList = path + "siemens/bom/fragment/list";
    const importUrl = path + "siemens/bom/import";
    $(function () {
        searcher();
    });
    ...
    const saveFragmentUrl = path + "siemens/bom/fragment/add";

    //加载图纸BOM
    function loadFragment(bomId) {
        $("#fragmentDg").datagrid("uncheckAll");
        const rows = $("#fragmentDg").datagrid("getRows");
        for (let i = 0; i < rows.length; i++) {
            $("#fragmentDg").datagrid("endEdit", i);
        }
        const inserted = $("#fragmentDg").datagrid('getChanges', "inserted");
        const deleted = $("#fragmentDg").datagrid('getChanges', "deleted");
        const updated = $("#fragmentDg").datagrid('getChanges', "updated");
    ...
    function saveFragment(tcBomId) {
        ...
        const fragmentGrid = {inserted: inserted, deleted: deleted, updated: updated};
        Loading.hide();
        Loading.show("正在保存");
        $.ajax({
            url: saveFragmentUrl,
    ...
    function onFragmentComboSelect(record) {
        ...
        const row = EasyUI.grid.getRowByIndex('fragmentDg', index);
    ...
    const importFragment = function () {
        ...
        const wid = Dialog.open("导入", 280, 280, importUrl, [EasyUI.window.button("icon-save", "保存", function () {
