...
<body class="easyui-layout" data-options="fit:true">
	<div region="west" split="true" title="BOM列表" style="width:260px;"
		...
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'north',split:false,border:false"
				class="datagrid-toolbar" style="height:27px;text-align: center;">
				<input id="searchInput" type="text" class="easyui-searchbox"
					prompt="请输入内容" searcher="searcher" editable="true"
					style="width:120px"> <label for="isSiemens"
					...
					<input id="isSiemens" type="checkbox" checked="checked"
					onchange="searcher()"
	...
	<div
		data-options="region:'center',border:false,title:'图纸BOM',iconCls:'platform-icon78'">
		<div id="drawings-toolbar">
			<a href="#" id="add" class="easyui-linkbutton"
				data-options="iconCls:'icon-add'" plain="true"
				onclick="addDrawings()">添加</a> <a href="#" id="del"
				class="easyui-linkbutton" data-options="iconCls:'icon-remove'"
				plain="true" onclick="deleteDrawings()">删除</a> <a href="#" id="edit"
				class="easyui-linkbutton" data-options="iconCls:'icon-edit'"
				plain="true" onclick="beginEdit()">编辑</a> <a href="#"
				id="cancelEdit" class="easyui-linkbutton"
				data-options="iconCls:'platform-icon8'" plain="true"
				onclick="cancelDrawingsEdit()">取消编辑</a> <a href="#" id="save"
				class="easyui-linkbutton" data-options="iconCls:'icon-save'"
				plain="true" onclick="saveDrawings()">保存</a> <a href="#"
				id="disable" class="easyui-linkbutton"
				data-options="iconCls:'platform-cancel_select'" plain="true"
				onclick="disableBom()">禁用</a> <a href="#" id="enable"
				class="easyui-linkbutton" data-options="iconCls:'platform-icon88'"
				plain="true" onclick="enableBom()">启用</a> <label class="save_tip">删除后保存生效</label>
		</div>
		<table id="drawingsDg" idField="id" singleSelect="false"
			...
			data-options="
					onLoadSuccess:onDrawingsDgLoadSuccess,
					onClickRow:onClickDrawingsDgRow
				">
			<thead>
				<tr>
					...
					<th field="fragmentName" width="23" sortable="true"
						data-options="editor:{type:'textbox',options:{required:false,readonly:true}}">名称</th>
					<th field="fragmentWeight" width="15" sortable="true"
						data-options="editor:{type:'textbox',options:{required:false,readonly:true}}">重量</th>
					<th field="fragmentLength" width="15" sortable="true"
						data-options="editor:{type:'textbox',options:{required:false,readonly:true}}">长度(M)</th>
					<th field="fragmentWidth" width="15" sortable="true"
						data-options="editor:{type:'textbox',options:{required:false,readonly:true}}">宽度(MM)</th>
					<th field="farbicModel" width="15" sortable="true"
						data-options="editor:{type:'textbox',options:{editable:false,required:true,validType:'length[1,100]'}}">胚布规格</th>
					<th field="fragmentCountPerDrawings" width="10" sortable="true"
						data-options="editor:{type:'numberbox',options:{editable:true,required:true,min:0,max:999,precision:0}}">数量</th>
					<th field="fragmentDrawingNo" width="15" sortable="true"
						data-options="editor:{type:'textbox',options:{required:true,validType:'length[1,100]'}}">图号</th>
					<th field="fragmentDrawingVer" width="12" sortable="true"
						data-options="editor:{type:'textbox',options:{required:true,validType:'length[1,100]'}}">图纸版本</th>
					...
					<th field="suitCountPerDrawings" width="15" sortable="true"
						data-options="editor:{type:'numberbox',options:{required:true,min:1,max:999,precision:0}}">图内套数</th>
					<th field="printSort" width="15" sortable="true"
						data-options="editor:{type:'numberbox',options:{required:true,min:1,max:9999,precision:0}}">出图顺序</th>
					<th field="fragmentMemo" width="20" sortable="true"
						data-options="editor:{type:'textbox',options:{editable:true,validType:{length:[0,100]}}}">备注</th>
				</tr>
			</thead>
		</table>
	</div>
	...
	<div id="dlg" class="easyui-dialog" title="图号和小部件数量确认"
		...
		data-options="
            	...
            	resizable:true,
            	closed:true,
                iconCls: 'platform-paste',
                buttons: '#dlg-buttons',
                modal:true
            ">
		<table id="summaryDg" class="easyui-datagrid" width="100%"
