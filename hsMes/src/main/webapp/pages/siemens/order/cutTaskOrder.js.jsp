...
<script>
    //添加裁剪派工单
    const addUrl = path + "siemens/cutTaskOrder/add";
    //编辑裁剪派工单
    const editUrl = path + "siemens/cutTaskOrder/edit";
    //删除裁剪派工单
    const deleteUrl = path + "siemens/cutTaskOrder/delete";
    const closeUrl = path + "siemens/cutTaskOrder/close";
    ...
    function setCount2(nv) {
        let v = nv;
        v = parseInt(v);
        const rows = $("#drawingsDg2").datagrid("getRows");
        const c = undefined;
        for (let i = 0; i < rows.length; i++) {
            /* if(v%rows[i].suitCountPerDrawings!=0){
                return;
    ...
    function exportTask() {
        location.href = path + "excel/export/裁剪派工单/com.bluebirdme.mes.siemens.excel.CutTaskOrderExportHandler/filter?" + JQ.getFormAsString("cutTaskOrderSearchForm");
    }
    ...
    function doPrintBarcode() {
        const r = EasyUI.grid.getOnlyOneSelected("dg");
        if (r == null) return;
        const p = $("#printer").combobox("getValue");
        if (p === "") {
            Tip.warn("请选择打印机");
            return;
        }
        const c = $("#printSuitCount").numberspinner("getValue");
        if (c === "") {
            Tip.warn("请输入打印套数");
            return;
        }
        const rs = EasyUI.grid.getSelections("drawingsDgPrint");
        if (rs.length === 0) {
            Tip.warn("请选择图号");
            return;
        }
        ...
        Loading.show("正在打印");
        JQ.ajaxPost(path + "siemens/cutTaskOrder/printBarcode", {
            ctoId: r.ID,
            cutPlanId: r.CUTPLANID,
            drawingsNo: dns.join(","),
            suitCount: c,
            printer: p
        }, function (data) {
    ...
    function doRePrint() {
        const printer = $("#rePrinter").combobox("getValue");
        if (printer === "") {
            Tip.warn("请选择打印机");
            return;
        }
        const rePrintCount = $("#rePrintCount").numberspinner("getValue");
        if (rePrintCount === "") {
            Tip.warn("请输入打印数量");
            return;
        }
        const reason = $("#reason").val();
        const ctoId = $("#ctoId").val();
        const dwId = $("#dwId").val();
        if (reason === "") {
            Tip.warn("请输入重打原因");
            return;
        }
        JQ.ajaxPost(path + "siemens/cutTaskOrder/rePrintBarcode", {
            ctoId: ctoId,
            dwId: dwId,
