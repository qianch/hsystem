...
<script>
    const produceTreeUrl = path + "totalStatistics/listProduct";
    const findInfoUrl = path + "mobile/common/infos";
    const findBarcodeInfoUrl = path + "mobile/common/barcodeInfos";
    const changeUrl = path + "totalStatistics/changeInfo";
    const changeMeterUrl = path + "totalStatistics/changeMeterInfo";
    const changeMemoUrl = path + "totalStatistics/changeMemoInfo";
    const abandonUrl = path + "mobile/stock/product/abandon2";
    //导出到ExcelUrl
    const exportUrl = path + "totalStatistics/export";
    let stt = "";
    ...
    const searchInfo = function () {
        ...
        const barcode = t.toString().trim();
        const url = produceTreeUrl + "?barcode=" + barcode;
        JQ.ajax(url, "get", null, function (data) {
            $('#produceTree').tree(
                {
                    ...
                    onClick: function (node) {
                        $("#infos").empty();
                        stt = "";
                        const barcode = node.id.toUpperCase().trim();
                        //点击时获取节点id，查询对应条码的产品信息
                        JQ.ajax(findInfoUrl + "?barCode=" + barcode, "get", null, function (ajaxData) {
            ...
            expandAll();
            const node = $('#produceTree').tree('find', barcode);
    ...
    function barcodeInfos(barcode) {
        JQ.ajax(findBarcodeInfoUrl + "?barCode=" + barcode, "get",
            null, function (ajaxData) {
                let str = "条    码    号 ：" + ajaxData.BARCODE + "\n";
                str += "订    单    号 ：" + ajaxData.SALESORDERCODE + "\n";
                str += "批    次    号 ：" + ajaxData.BATCHCODE + "\n";
                const pInfos = (ajaxData.OUTPUTSTRING).split("\t");
                str += "厂内产品名称：" + pInfos[0] + "\n";
                str += "客户产品名称：" + pInfos[1] + "\n";
    ...
    function initProduct(data, barcode) {
        ...
        const batchCode = data.CODE.BATCHCODE;
        let deviceCode = "";
        //判断条码类型
        ...
        stt += "<td><input type=\"text\" class=\"w\" id='weight' readonly='true' value='" + numberFormatter(weight) + "' > ";
        ...
        if (barcode.substr(0, 1) === "P") {
            if ((!data.PACKED) ||
                ...
                (data.PACKED && data.TRAY == null)) {
                stt += "<input type=\"button\" id=\"editbtn\" class=\"modifybtn\" value=\"修改\" onclick=\"changeInfo('" + barcode + "')\">";
                stt += "<input type=\"button\" id=\"savebtn\" class=\"savebtn\" value=\"保存\" style=\"display:none;\" onclick=\"saveInfo('" + barcode + "')\">";
            }
        }
        stt += "</td>";
        stt += "</tr>";
        if ((barcode.indexOf("R") === 0 || barcode.indexOf("P") === 0) && data.ROLL != null) {
            ...
            stt += "<td><input type=\"text\" class=\"w\" id='meter' readonly='true' value='" + data.ROLL.ROLLREALLENGTH + "' > ";
            if (barcode.substr(0, 1) === "R") {
                stt += "<input type=\"button\" id=\"editbtnM\" class=\"modifybtn\" value=\"修改\" onclick=\"changeInfoM('" + barcode + "')\">";
                stt += "<input type=\"button\" id=\"savebtnM\" class=\"savebtn\" value=\"保存\" style=\"display:none;\" onclick=\"saveInfoM('" + barcode + "')\">";
            }
        }
        stt += "</td>";
        stt += "</tr>";
        ...
        stt += "<td>" + (isEmpty(ppd) ? data.PRODUCT.PRODUCTWIDTH : ppd.PRODUCTWIDTH) + "mm</td>";
        stt += "</tr>";
        stt += "<tr>";
        stt += "<td class='title'>定长/定重</td>";
        let length = (isEmpty(ppd) ? (data.PRODUCT.PRODUCTROLLLENGTH) : (ppd.PRODUCTLENGTH));
        weight = (isEmpty(ppd) ? (data.PRODUCT.PRODUCTROLLWEIGHT) : (ppd.PRODUCTROLLWEIGHT));
        if (length == null) {
            length = 0.00;
        }
        if (weight == null) {
        ...
        stt += "<td><input type=\"textarea\" class=\"memo\"  id='memo' readonly='true' value='" + data.MEMO + "' > ";
        stt += "<input type=\"button\" id=\"editbtnMemo\" class=\"modifybtn\" value=\"修改\" onclick=\"changeInfoMemo('" + barcode + "')\">";
        stt += "<input type=\"button\" id=\"savebtnMemo\" class=\"savebtn\" value=\"保存\" style=\"display:none;\" onclick=\"saveInfoMemo('" + barcode + "')\">";
        stt += "</td>";
        stt += "</tr>";
    }
    ...
    function changeInfoM(data) {
        const editbtn = document.getElementById("editbtnM");
        const savebtn = document.getElementById("savebtnM");
        const weightText = document.getElementById("meter");
        editbtn.style.display = "none";
        savebtn.style.display = "";
        weightText.readOnly = false;
        weightText.focus();
    ...
    function changeInfoMemo(data) {
        const editbtn = document.getElementById("editbtnMemo");
        const savebtn = document.getElementById("savebtnMemo");
    ...
    function saveInfo(data) {
        ...
        const topBarcode = root.id;
        const parentNode = t.tree('getParent', node.target);
        let parentBarocde;
        const barcode = node.id;
    ...
    function Export() {
        //搜索框值
        const searchInput = $("#searchInput").textbox("getValue");
