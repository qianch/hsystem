...
<body>
    <!-- 后加的退出/进入全屏 -->
    <div onclick="fullScreen()" style="z-index:999;position: fixed;bottom:0px;right:0px;height:40px;line-height:40px;text-align:center;font-weight:bold;font-size:18px;font-family:'微软雅黑';width:200px;background:#80808094;">
                进入/退出 全屏
    </div>
    <div id="closeButton" onclick="closeWin()" style="position: fixed; top: 5px; right: 5px; background: red; color: white; font-weight: bold; width: 60px; text-align: center; height: 30px; line-height: 30px; border-radius: 5px;">关闭</div>
    ...
    <table class="tb_header" border="1" cellspacing="0" cellpadding="0" bordercolor="gray" style="BORDER-COLLAPSE: collapse;">
        <tr>
    ...
    <script id="orders" type="text/template">
        {{each data as value i}}
                <tr style="border-top:3px solid #6f6f6f;border-bottom:3px solid #6f6f6f;" data-row="true" class="{{colorful(i)}} {{producing(value.ISPRODUCING)}} {{prior(value.SORT)}}">
                    <td>{{value.CONSUMERPRODUCTNAME}}<hr>{{value.PRODUCTMODEL}}</td>
                    <td>{{nullcontent(value.PRODUCTWIDTH)}}毫米<hr>{{nullcontent(value.PRODUCTLENGTH)}}米<hr>{{nullcontent(value.PRODUCTWEIGHT)}}千克</td>
                    <td>{{nullcontent(value.ROLLNO)}}<hr>{{nullcontent(value.LEVELNO)}}<hr>{{nullcontent(value.DRAWINGNO)}}</td>
                    <td>
                        <button data-clipboard-text="{{code(value.PROCESSBOMCODE)}} {{value.PROCESSBOMVERSION}}">{{code(value.PROCESSBOMCODE)}}({{value.PROCESSBOMVERSION}})</button>
                        <hr><button data-clipboard-text="{{code(value.BCBOMCODE)}} {{nullcontent2(value.BCBOMVERSION)}}">{{code(value.BCBOMCODE)}}({{nullcontent(value.BCBOMVERSION)}})</button>
                    </td>
