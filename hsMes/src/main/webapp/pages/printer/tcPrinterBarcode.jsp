...
<script type="text/javascript">
	//JS代码
var getCount = function(newValue,oldValue){
	var r = EasyUI.grid.getOnlyOneSelected("dailyDetails");
	JQ.ajaxPost(path + "bom/plan/tc/ver/parts/"+ + r.FROMSALESORDERDETAILID + "/"+r.ID+ "/"+newValue, {
    }, function(data) {
...
<div>
	...
	<form id="doPrintBarcodeForm" method="post" ajax="true"
		...
		<input	type="hidden" name="cutDailyPlanId" value="${cutDailyPlanId}" />
		<input type="hidden" name="departmentName" value="${departmentName}" />
		<%-- 		<input type="hidden" name="printerId" value="${printer.id}" /> <input
			type="hidden" name="salesOrderId" value="" /> --%>
		<table width="100%">
			<tr>
			...
			<td class="title">打印部件:</td>
				<td><input class="easyui-combobox"
					style="width:95%;"
					data-options="valueField:'value',url:'<%=basePath%>planner/cutPlan/findParts2?cutPlanId=${cutPlanId}&&cutDailyPlanId=${cutDailyPlanId}',onSelect: function(rec){
        ...
        "
                    ...
                    <input   type="hidden" name="partName"  id="partName" /></td>
			...
			<tr>
				...
				<td>
					<!-- <input name="count" id="count" class="easyui-numberspinner" style="width:265px;"
       				 ...
       				 data-options="precision:0" style="width:265px;" required="true"></input>
				</td>
			</tr>
			<tr>
				...
				<td>
					<!-- <input name="count" id="count" class="easyui-numberspinner" style="width:265px;"
       				 required="required" data-options="min:1,max:100,editable:false,value:1">   -->
					<input name="copies" id="copies" type="text" class="easyui-numberbox" value=${copies==null?"4":copies}
							data-options="precision:0" style="width:265px;" required="true"></input>
				</td>
			</tr>
			<tr>
