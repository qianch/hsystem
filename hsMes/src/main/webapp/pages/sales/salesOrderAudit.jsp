...
<style type="text/css">
@keyframes changed {
	from {color: white;background: rgb(214, 9, 9);}
	to {color: white;background: rgba(255, 0, 0, 0.55);}
}
</style>
...
<div>
	<!--销售订单表单-->
	<form id="salesOrderAuditForm" method="post" ajax="true" action="<%=basePath %>salesOrder/${empty salesOrder.id ?'add':'edit'}" autocomplete="off">
		...
		<table width="100%">
			<tr>
				...
				<!--订单号-->
				<td><input type="text" id="salesOrderCode" name="salesOrderCode" value="${salesOrder.salesOrderCode}" class="easyui-textbox" required="true" editable="false" data-options="'icons':[]"></td>
				<td class="title">下单日期:</td>
				<!--下单日期-->
				<td><input type="text" id="salesOrderDate" name="salesOrderDate" value="${fn:substring(salesOrder.salesOrderDate, 0, 19)}" class="easyui-textbox" required="true" editable="false" data-options="'icons':[]"></td>
			</tr>
			<tr>
				...
				<td><input type="hidden" name="salesOrderConsumerId" id="salesOrderConsumerId" value="${salesOrder.salesOrderConsumerId }"> <input type="hidden" id="consumerCode" value="${consumerCode }"> <input type="text" id="salesOrderConsumerName" value="${consumerName}"
					class="easyui-textbox" required="true" editable="false" data-options="'icons':[]"></td>
				<td class="title">业务员:</td>
				<!--业务员-->
				<td><input type="hidden" name="salesOrderBizUserId" id="salesOrderBizUserId" value="${salesOrder.salesOrderBizUserId }"> <input type="text" id="salesOrderBizUserName" value="${userName}" class="easyui-textbox" required="true" editable="false" data-options="'icons':[]"></td>
			</tr>
			<tr>
				<td class="title">内销/外销/胚布:</td>
				<!--内销/外销-->
				<td><input type="text" id="salesOrderIsExport" name="salesOrderIsExport" value="${salesOrder.salesOrderIsExport}" class="easyui-combobox" required="true" editable="false" readonly="true" data-options="valueField:'v',textField:'t',data:[{'v':'1','t':'内销'},{'v':'-1','t':'胚布'},{'v':'0','t':'外销'}],'icons':[]"></td>
				<td class="title">订单类型:</td>
				<!--订单类型 （3新品，2试样，1常规产品，-1未知）-->
				<td><input type="text" id="salesOrderType" required="true" name="salesOrderType" value="${salesOrder.salesOrderType}" class="easyui-combobox" required="true" readonly="true" editable="false"
					data-options="valueField:'v',textField:'t',data:[{'v':'1','t':'常规'},{'v':'2','t':'试样'},{'v':'3','t':'新品'},{'v':'-1','t':'未知'}],'icons':[]"></td>
			...
			<tr>
				...
				<!--订单备注-->
				<td colspan="3">
					<textarea style="width:100%;height:50px;border-radius:5px;background-color:white;" name="salesOrderMemo"
		...
		<table id="audit_product_dg" title="订单产品" class="easyui-datagrid" fitColumns="false" style="width:100%;"
			...
			<thead>
				<tr>
					...
					<th width="80" field="TOTALWEIGHT" data-options="formatter:function(value,row,index){if(isEmpty(value))return '';return value+'KG';}">总重量</th>
					<th width="80" field="TOTALMETRES" data-options="formatter:function(value,row,index){if(isEmpty(value))return '';return value+'米';}">总米长</th>
