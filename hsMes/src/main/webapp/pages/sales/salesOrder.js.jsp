...
<script>
                ...
                $("#ddv-p-" + index).datagrid({
                    ...
                    columns: [[
                        ...
                        },
                        {
                            ...
                            field: 'PRODUCTCONSUMERBOMVERSION',
                            width: 120,
                            title: '客户版本'
                        }, {
            ...
            if ($("#product_dg").datagrid("getRows").length !== 0) {
                if (endEdit()) {
                    ...
                    for (let i = 0; i < order.details.length; i++) {
                        order.details[i].salesOrderSubCode = order.details[i].salesOrderSubCode.replace(new RegExp(" ", "gm"), "");
                        order.details[i].salesOrderSubCode = order.details[i].salesOrderSubCode.replace(new RegExp("    ", "gm"), "");
                        order.details[i].salesOrderSubCodePrint = order.details[i].salesOrderSubCodePrint.replace(new RegExp(" ", "gm"), "");
                        order.details[i].salesOrderSubCodePrint = order.details[i].salesOrderSubCodePrint.replace(new RegExp("    ", "gm"), "");
    ...
    function selectProduct() {
        ...
        selectProductWindowId = Dialog.open("选择产品", 900, 400, path + "selector/product?singleSelect=false&showCode=false&isShow=1", [EasyUI.window.button("icon-ok", "选择", function () {
            ...
            for (i = 0; i < rows.length; i++) {
                if (rows[i].CARRIERCODE.indexOf("04.03") !== -1 && rows[i].CARRIERWEIGHT === 0) {
                    Tip.warn("请维护纸管" + rows[i].CARRIERCODE + "重量");
                    return;
                }
            }
    ...
    function addToProductDg(r) {
        const _row = {
            ...
            "productConsumerBomVersion": r.PRODUCTCONSUMERBOMVERSION,
    ...
    const doAudit = function () {
        ...
        const wid = Dialog.open("审核", 370, 120, _auditCommitUrl + "?id=" + r.ID, [EasyUI.window.button("icon-ok", "提交审核", function () {
