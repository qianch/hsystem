...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#dg_ftcBom_detail").datagrid("reloadFooter",[{"FTCBOMDETAILWEIGHTPERSQUAREMETRE":FTCBOMDETAILTOTALWEIGHT,"FTCBOMDETAILITEMNUMBER":"单位面积质量合计(g/m²)"}]);
	});
...
<body class="easyui-layout">
	<div data-options="region:'north'" style="height:78px">
		<!--非套材bom版本信息表单-->
		<form id="ftcBomVersionForm" method="post" ajax="true" action="<%=basePath %>ftcBomVersion/${empty bCBomVersion.id ?'add':'edit'}" autocomplete="off" autoSearchFunction="false">

			<input type="hidden" name="id" value="${bCBomVersion.id}" /> <input id="packBomId" type="hidden" name="packBomId" value="${bCBomVersion.packBomId}" />
			<table width="100%">
				<tr>
					<td class="title">版本号:</td>
					<!--版本号-->
					<td><input type="text" id="packVersion" name="packVersion" value="${ftcBomVersion.ftcProcBomVersionCode}" class="easyui-textbox" readonly="true"></td>
				</tr>
				<%-- <tr>
	...
	<div data-options="region:'center'">
		<table id="dg_ftcBom_detail" singleSelect="false" title="非套材Bom明细" class="easyui-datagrid" url="" rownumbers="true" fitColumns="true" fit="true" showFooter="true">
			<thead>
				<tr>
