...
<script>
    ...
    const saveUrl = path + "bom/packaging/saveDetail";
    //删除包材bom明细
    const deleteUrl = path + "bom/packaging/delete";
    let bcBomtreeUre = path + "bom/packaging/listBom";
    let isFirst = true;
    const commitAudit = path + "selector/commitAudit";
    const commitUrl = path + "bom/packaging/commit";
    //查看bom下有没有版本
    const findV = path + "bom/packaging/findV";
    ...
    //左边的tree
    const addVersionUrl = path + "bom/packaging/addBomVersion";
    const editVersionUrl = path + "bom/packaging/editBomVersion";
    const deleteVersionUrl = path + "bom/packaging/deleteBomVersion";
    ...
    function appenditVersion() {
        ...
        JQ.ajax(findV, "post", {
            ...
            const wid = Dialog.open("添加", 380, 150, addVersionUrl, [EasyUI.window.button("icon-save", "保存", function () {
    ...
    const copyBomUrl = path + "bom/packaging/copyBom";
    ...
    function edititVersion() {
        const t = $('#BcBomTree');
        const node = t.tree('getSelected');
        const wid = Dialog.open("编辑", 380, 250, editVersionUrl + "?id=" + node.id, [EasyUI.window.button("icon-save", "保存", function () {
    ...
    function reload1() {
        isgetTestPro();
        const node = $("#BcBomTree").tree("getParent", $('#BcBomTree').tree('getSelected').target);
        if (node) {
    ...
    function commit() {
        const r = $('#BcBomTree').tree('getSelected');
        const id = r.id;
        dialogId = Dialog.open("审核", 700, 100, commitAudit + "?id=" + id, [EasyUI.window.button("icon-save", "提交审核", function () {
            ...
            const node = $('#BcBomTree').tree('getSelected');
            const pNode = $('#BcBomTree').tree('getParent', node.target);
    ...
    function appendit() {
        const wid = Dialog.open("添加", 900, 380, path + "/bom/packaging/addBom", [EasyUI.window.button("icon-save", "保存", function () {
    ...
    function editit() {
        const t = $('#BcBomTree');
        const node = t.tree('getSelected');
        const wid = Dialog.open("编辑", 380, 380, path + "/bom/packaging/editBom" + "?id=" + node.id, [EasyUI.window.button("icon-save", "保存", function () {
    ...
    const bomDetailExcel = path + "bom/packaging/upload";
    ...
    function excel(id) {
        ...
        //提交导入文件内容
        const cutTcBomMainUploadFileUrl = path + "bom/packaging/importBcBomUploadFile";
        if (data != null) {
    ...
    //常规产品
    const cgUrl = path + "audit/BC/{id}/state";
    //变更试样
    const syUrl = path + "audit/BC1/{id}/state";
    //新品试样
    const syUrl1 = path + "audit/BC2/{id}/state";
            ...
            $('#BcBomTree').tree({
                ...
                onClick: function (node) {
                    ...
                    if (detail.nodeType === "bom") {
                        ...
                        const pNode = $(this).tree('getParent', node.target);
                        detail = pNode.attributes;
                        $('#PACKBOMGENERICNAME').html(detail.PACKBOMGENERICNAME);
                        $('#PACKBOMNAME').html(detail.PACKBOMNAME);
                        $('#PACKBOMCODE').html(detail.PACKBOMCODE);
        ...
        $('#BcBomTree').tree({
            ...
            onContextMenu: function (e, node) {
                ...
                if (node.attributes.nodeType === "root") {
                    ...
                    const item1 = $('#treeMenuVersion').menu('findItem', '修改');
                    const item2 = $('#treeMenuVersion').menu('findItem', '删除');
                    let item3 = $('#treeMenuVersion').menu('findItem', '工艺变更');
                    if (node.attributes.AUDITSTATE === 1) {
                        $('#treeMenu').menu('hideItem', item.target);
                        $('#treeMenu').menu('hideItem', item1.target);
                        $('#treeMenu').menu('hideItem', item2.target);
