...
<script type="text/javascript">
    //添加成品信息
    const find = path + "bom/tc/findFtc";
    //添加成品信息
    const _common_product_addUrl = path + "finishProduct/add";
    const _common_product_editUrl = path + "finishProduct/edit";
    //选择客户信息
    ...
    const findConsumer = path + "bom/tc/addConsum11er";
    const findProduct = path + "bom/tc/findProduct";
    let consumerWindow = null;
    let bomWindow = null;
    let packingBomWindow = null;
    let productIsTc = null;
    const consumerName = null;
    const consumerCode = null;
    ...
    function ChooseConsumer1() {
        consumerWindow = Dialog.open("选择客户", 850, 450, findConsumer, [EasyUI.window.button("icon-save", "确认", function () {
            const _product_r = EasyUI.grid.getOnlyOneSelected("dddg");
    ...
    function copy() {
        ...
        const wid = Dialog.open("复制胚布信息", 700, 295, _common_product_editUrl + "?id=" + r.ID + "&copy=true", [
...
<div id="_common_product_toolbar1">
    <form action="#" id="partProductSearchForm" autoSearch="true" autoSearchFunction="true">
        ...
        工艺代码：<input type="text" name="filter[productProcessCode]" like="true" class="easyui-textbox"
                        style="width:230px;"></br>
        工艺版本：<input type="text" name="filter[productProcessbomVersion]" like="true" class="easyui-textbox"
                        style="width:230px;">
    ...
    <div style="border-top:1px solid #DDDDDD">
        <a href="javascript:void(0)" class="easyui-linkbutton" plain="true" iconcls="platform-copy"
           onclick="copy()">复制</a>
    </div>
</div>
<table id="dg" idField="ID" style="width:100%" singleSelect="false" class="easyui-datagrid"
       url="${path}bom/tc/findFtc" toolbar="#_common_product_toolbar1" pagination="true" remoteSort="false"
       rownumbers="true" fitColumns="true" fit="true" remoteFilter="true">
    <thead>
    <tr>
