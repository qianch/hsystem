...
<script>
    const e = null;
    const addFtcBom = path + "bom/tc/addFtcBom";
    const findparts = path + "bom/tc/find";
    //添加套材BOM
    const addUrl = path + "bom/tc/add";
    //编辑套材BOM
    const editUrl = path + "bom/tc/edit";
    //删除套材BOM
    const deleteUrl = path + "bom/tc/delete";
    //查询bom信息
    let findBom = path + "bom/tc/findBom";
    //添加BOM版本
    const addV = path + "bom/tcBomVersion/add";
    //编辑BOM版本
    const editV = path + "bom/tcBomVersion/edit";
    //删除套材BOM版本
    const deleteV = path + "bom/tc/deleteV";
    //保存部件
    const savePartsUrl = path + "bom/tc/saveParts";
    //校验部件是否可删除
    const delValid = path + "bom/tc/delValid";
    //删除部件
    const deleteP = path + "bom/tc/deleteDetail";
    //查询部件信息
    const partsList = path + "bom/tcBomVersionParts/list";
    //查询部件明细
    const partsD = path + "bom/tcBomVersionPartsDetail/list";
    //删除部件明细
    const deleteD = path + "bom/tcBomVersionPartsDetail/delete";
    //保存部件明细
    const saveDetailUrl = path + "bom/tc/saveDetail";
    //查询部件成品重量胚布信息
    const partsFW = path + "bom/tcBomVersionPartsFinishedWeightEmbryoCloth/list";
    //删除部件成品重量胚布信息
    const deleteFW = path + "bom/tcBomVersionPartsFinishedWeightEmbryoCloth/delete";
    //保存部件成品重量胚布信息
    const saveFWUrl = path + "bom/tc/saveFinishedWeightEmbryoCloth";
    //复制树
    const copyBomVersionUrl = path + "bom/tc/copytcBomVersion";
    const copyBomUrl = path + "bom/tc/copytcBom";
    //审核页面
    const commitAudit = path + "bom/tcBomVersion/commitAudit";
    //添加成品信息
    const _common_product_addUrl = path + "finishProduct/add";
    //选择客户信息
    const chooseConsumer = path + "selector/consumer";
    //根据版本查询审核信息
    const findCode = path + "bom/tc/findB";
    //打开提交审核页面
    const _auditCommitUrl = path + "selector/commitAudit";
    //查询此部件是否有明细
    const findDetial = path + "bom/tc/findDetial";
    //查看部件的版本的审核状态
    const findA = path + "bom/tc/findA";
    //查看bom下有没有版本
    const findV = path + "bom/tc/findV";
    let consumerWindow = null;
    let isFirst = true;
    let nodeID = null;
    let newNode = null;
    let editIndex = undefined;
    let editIndexD = undefined;
    let editIndexFW = undefined;
    let addindex = 0;
    let addindexI = 0;
    let addindexF = 0;
    const indexA = 0;
        ...
        $('#TcBomTree').tree({
            ...
            onContextMenu: function (e, node) {
                ...
                if (node.attributes.status === "0") {
                    ...
                    const item = $('#treeMenu').menu('findItem', '提交审核');
                    const item1 = $('#treeMenu').menu('findItem', '修改');
                    const item2 = $('#treeMenu').menu('findItem', '删除');
                    const item3 = $('#treeMenu').menu('findItem', '工艺变更');
            ...
            onClick: function (node) {
                ...
                if (node.attributes.status === "3") {
                    const node1 = $('#TcBomTree').tree('find', node.attributes.TCPROCBOMVERSIONPARENTPARTS);
                    $("#parts").datagrid({
    ...
    function reload1() {
        isgetTestPro();
        const node = $("#TcBomTree").tree("getParent", $('#TcBomTree').tree('getSelected').target);
        if (node) {
            $('#TcBomTree').tree('reload', node.target);
        } else {
            $('#TcBomTree').tree('reload');
    ...
    //右击添加工艺bom
    function appendit() {
        const wid = Dialog.open("添加套材工艺BOM", 550, 300, addUrl, [EasyUI.window.button("icon-save", "保存", function () {
            if (cc != null) {
                try {
                    cc.window('close');
                    cc = null;
    ...
    function editVV() {
        const node = $("#TcBomTree").tree('getSelected');
        const wid = Dialog.open("编辑套材工艺BOM", 600, 200, editUrl + "?id=" + node.id, [EasyUI.window.button("icon-save", "保存", function () {
    ...
    function appendV() {
        ...
        JQ.ajax(findV, "post", {
            ...
            const wid = Dialog.open("添加套材工艺版本", 600, 100, addV + "?id=" + node.id, [EasyUI.window.button("icon-save", "保存", function () {
    ...
    function editB() {
        const t = $('#TcBomTree');
        const node = t.tree('getSelected');
        const wid = Dialog.open("编辑套材工艺版本", 600, 200, editV + "?id=" + node.id, [EasyUI.window.button("icon-save", "保存", function () {
            if (cc != null) {
                try {
                    cc.window('close');
                    cc = null;
    ...
    //常规
    const viewUrl = path + "audit/TC/{id}/state";
    //变更试样
    const viewUrl1 = path + "audit/TC1/{id}/state";
    //新品试样
    const viewUrl2 = path + "audit/TC2/{id}/state";
    ...
    function onEndEdit(index, row, changes) {
        let rowstr;
        const rs = $("#parts").datagrid("getRows");
        const names = {};
        for (let i = 0; i < rs.length; i++) {
            const name = rs[i].TCPROCBOMVERSIONPARTSNAME;
    ...
    //删除部件datagrid数据
    const doDelete1 = function () {
        const r = EasyUI.grid.getSelections("parts");
        if (r.length === 0) {
    ...
    function onDblClickCellD(index, field) {
        ...
        if (editIndexD !== undefined && editIndexD !== index) {
            if (endEditingD()) {
                ...
                const ed = $('#partsDetails').datagrid('getEditor', {
                    index: index,
                    field: field
                });
                if (ed) {
    ...
    function contains(gridId, rowData) {
        const data = $("#" + gridId).datagrid("getData");
        if (data.total === 0) {
            return false;
        } else {
    ...
    const doDelete = function () {
        const r = EasyUI.grid.getSelections("partsDetails");
        if (r.length === 0) {
            Tip.warn("<spring:message code="Tip.SelectAtLeastOne" />");
            return;
        }
    ...
    function audit() {
        const node = $('#TcBomTree').tree('getSelected');
        const wid = Dialog.open("审核", 700, 120, _auditCommitUrl + "?id=" + node.id, [EasyUI.window.button("icon-ok", "提交审核", function () {
    ...
    const addFW = function () {
        ...
        const children = $("#TcBomTree").tree("getChildren", node.target);
        if (children.length > 0) {
    ...
    const editFW = function () {
        $("#partsFinishedWeightEmbryoCloth").datagrid('endEdit', editIndexFW);
        addindexF = 0;
    };

    //删除部件成品重量胚布信息datagrid数据
    const doDeleteFW = function () {
        const r = EasyUI.grid.getSelections("partsFinishedWeightEmbryoCloth");
        if (r.length === 0) {
            Tip.warn("<spring:message code="Tip.SelectAtLeastOne" />");
            return;
        }
    ...
    function onDblClickCellFW(index, field) {
        ...
        if (editIndexFW !== undefined && editIndexFW !== index) {
            if (endEditingFW()) {
                ...
                const ed = $('#partsFinishedWeightEmbryoCloth').datagrid('getEditor', {
                    index: index,
                    field: field
                });
                if (ed) {
