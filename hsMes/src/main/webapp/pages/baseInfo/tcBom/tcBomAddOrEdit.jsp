...
<div>
	<!--非套材工艺BOM表单-->
	<form id="tc_BomForm" method="post" ajax="true" action="<%=basePath %>tcBom/${empty tcBom.id ?'add':'edit'}" autocomplete="off" >
		
		<input type="hidden" name="id" value="${tcBom.id}" />
		<!--客户信息ID-->
		<input type="hidden" id="ftcProcBomConsumerId" name="tcProcBomConsumerId" value="${tcBom.tcProcBomConsumerId}" />
		<table width="100%">
				<tr>
					<td class="title">BOM名称:</td>
					<!--BOM名称-->
					<td>
						<input type="text" data-options="required:true,validType:'length[1,100]'" id="ftcProcBomName" name="tcProcBomName" value="${tcBom.tcProcBomName}" class="easyui-textbox" style="width:100%;"  >
					</td>
				</tr>
				<tr>
					...
					<!--BOM代码-->
					<td>
						<input type="text" id="ftcProcBomCode" data-options="required:true,validType:'length[1,100]'" name="tcProcBomCode" value="${tcBom.tcProcBomCode}" class="easyui-textbox" style="width:100%;"  >
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<input id="ftcProcBomConsumer" class="easyui-searchbox" value="${consumer.consumerName}" editable="false" required="true" data-options="searcher:ChooseConsumer"  style="width:100%;">
					</td>
				</tr>
			<tr>
					...
					<td>
						<%-- <input type="text" id="ftcProcBomConsumerId" name="ftcProcBomConsumerId" value="${fTc_Bom.ftcProcBomConsumerId}" class="textbox" required="true" > --%>
						<input type="text" id="isTestPro" name="isTestPro" value="${tcBom.isTestPro}" class="easyui-combobox" style="width: 100%" panelHeight="auto" editable="false" required="true" data-options="data: [
	                        {value:'0',text:'常规产品'},
	                        {value:'1',text:'变更试样'},
	                        {value:'2',text:'新品试样'}
                    	]" >
				...
				<c:if test="${empty tcBom.id}">
					<tr>
						<td class="title">版本号:</td>
						<!--版本号-->
						<td>
							<input type="text" id="tcProcBomVersionCode" name="tcProcBomVersionCode" style="width: 100%" class="easyui-textbox" data-options="validType:['length[0,100]']">
						</td>
					</tr>
					<tr>
						...
						<!--客户版本号-->
						<td>
							<input type="text" data-options="validType:'length[0,100]'" id="tcConsumerVersionCode" name="tcConsumerVersionCode" class="easyui-textbox" style="width: 100%" >
						</td>
					</tr>
					<tr>
...
// 		uploadLimit		:	1,			//上传文件的数量
		...
		onUploadSuccess : function(file, data, response) {
			...
			$("#tcProcBomVersionCode").textbox({required:true});
				$("#fileId").val(data.ID);
	...
	Dialog.confirm(function(){
		$.ajax({
			...
			success : function(data) {
				...
				Tip.success("删除成功");
				$("#fileId").val('');
				$("#tcProcBomVersionCode").textbox({required:false});
