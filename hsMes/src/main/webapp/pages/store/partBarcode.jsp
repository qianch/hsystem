...
<head>
    ...
    <%@ include file="partBarcode.js.jsp" %>
</head>
...
<div data-options="region:'center',border:false" style="position: relative; height: 140px; width: 925px">
    <div id="toolbar">
        <div style="border-top:1px solid #DDDDDD">
            <form action="#" id="partBarcodeSearchForm" autoSearchFunction="false">
                选择打印机：<input id="pName" class="easyui-combobox" name="pName" required="true"
                ...
                <a href="javascript:void(0)" class="easyui-linkbutton l-btn l-btn-small" iconcls="icon-search"
                   onclick="filter()">搜索</a>
                <a href="javascript:void(0)" class="easyui-linkbutton" iconcls="icon-print" onclick="print()">补打条码</a>
                <div class="datagrid-toolbar"></div>
                <br/>
                任务单号： <input type="text" name="filter[producePlanCode]" class="easyui-textbox">
                批次号： <input type="text" name="filter[batchCode]" class="easyui-textbox">
                创建时间:<input type="text" id="start" name="filter[startTime]" class="easyui-datetimebox">-<input
                    type="text" id="end" name="filter[endTime]" class="easyui-datetimebox">
                <br/>客户名称:<input type="text" id="consumerName" name="consumerName" value="${btwFile.consumerName}"
                                 class="easyui-searchbox" required="true"
                                 data-options="searcher:selectConsumer,icons:[]">
                ...
                打印数量:<input id="printCount" class="easyui-numberbox" value="1" data-options="min:1,max:10,precision:0"
                            required="true">
                <a href="javascript:void(0)" class="easyui-linkbutton" iconcls="icon-print" onclick="doPrintPart()">补打部件条码</a>
                <input type="hidden" name="filter[consumerId]" id="consumerId" value="${btwFile.consumerId}">

            </form>
        </div>
        <div style="border-top:1px solid #DDDDDD">
            客户条码号数值:<input type="number" id="customerBarCodeRecord" name="customerBarCodeRecord" value="0"
                           class="easyui-textbox">
            供销商条码数值:<input type="number" id="agentBarCodeRecord" name="agentBarCodeRecord" value="0"
                           class="easyui-textbox">
            <a href="javascript:void(0)" class="easyui-linkbutton" iconcls="icon-edit" onclick="editPlanDetailPrints()">修改打印属性</a>
            <a href="javascript:void(0)" class="easyui-linkbutton" iconcls="icon-edit" onclick="editBacode()">修改条码</a>
            <a href="javascript:void(0)" class="easyui-linkbutton" iconcls="icon-remove" onclick="clearPart()">清空条码</a>
        </div>
    </div>
    <table id="dg" singleSelect="false" title="" class="easyui-datagrid" toolbar="#toolbar" pagination="true"
           rownumbers="true" fitColumns="true" fit="true" data-options="onLoadSuccess:dgLoadSuccess,
				remoteFilter:true">
        <thead>
        <tr>
            <th field="ID" checkbox=true></th>
            <th field="BARCODE" sortable="true" width="15">条码号</th>
            ...
            <th field="PARTNAME" sortable="true" width="15">产品部件名称</th>
