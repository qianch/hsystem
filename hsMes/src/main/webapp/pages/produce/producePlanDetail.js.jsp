...
<script>
//添加生产计划明细
var addUrl=path+"producePlanDetail/add";
//编辑生产计划明细
var editUrl=path+"producePlanDetail/edit";
//删除生产计划明细
var deleteUrl=path+"producePlanDetail/delete";

var dialogWidth=700,dialogHeight=350;

//查询
function filter() {
	EasyUI.grid.search("dg","producePlanDetailSearchForm");
}
...
var add = function() {
	var wid = Dialog.open("添加", dialogWidth, dialogHeight,addUrl, [
		...
		EasyUI.form.submit("producePlanDetailForm",addUrl, function(data){
			filter();
...
var edit = function(){
	...
	var wid = Dialog.open("编辑", dialogWidth, dialogHeight, editUrl+"?id="+r.ID, [ 
	    EasyUI.window.button("icon-save", "保存", function() {
		EasyUI.form.submit("producePlanDetailForm", editUrl, function(data) {
			filter();
			Dialog.close(wid);
		})
	}), EasyUI.window.button("icon-cancel", "关闭", function() {
