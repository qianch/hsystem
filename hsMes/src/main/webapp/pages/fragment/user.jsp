...
<script type="text/javascript">
    //点击的节点
    let _common_user_treeNode = null;
    //ztree 设置
    const _common_user_setting = {
        edit: {
            enable: true,
            showRemoveBtn: false,
            showRenameBtn: false
    ...
    function _common_user_zTreeOnClick(event, treeId, treeNode) {
        ...
        const ids = [];
        const id = _common_user_getChildren(ids, treeNode);
        ...
        <div id="_common_user_toolbar">
            <div style="border-top:1px solid #DDDDDD">
                <form action="#" id="_common_user_userFilter" autoSearchFunction="false">
                    <label class="panel-title">用户搜索：</label> <input type="hidden" id="_common_user_did"
                                                                        name="filter[u.did]" in="true"> 用户名：<input
                        type="text" id="_common_user_userName" name="filter[u.userName]" like="true" value=""
                        class="easyui-textbox"> 登录名称：<input type="text"
                                                                id="_common_user_loginName" name="filter[u.loginName]"
                                                                like="true" value="" class="easyui-textbox"> <a
                        href="javascript:void(0)" class="easyui-linkbutton l-btn l-btn-small" iconcls="icon-search"
                        onclick="_common_user_filter()"> 搜索 </a>
                </form>
            </div>
        </div>
        <table id="_common_user_dg" idField="ID" fit="true" singleSelect="${empty singleSelect?'true':singleSelect }"
               title="人员信息列表" url="<%=basePath%>user/list?all=1" toolbar="#_common_user_toolbar"
            ...
            <tr>
                ...
                <th field="STATUS" width="20" data-options="field:'status',formatter:_common_user_statusFormatter">
                    状态
                </th>
            </tr>
            </thead>
