...
<script>
    //添加投诉
    const addUrl = path + "complaint/add";
    //编辑投诉
    const editUrl = path + "complaint/edit";
    //删除投诉
    const deleteUrl = path + "complaint/delete";
    //获取序列号
    const serialUrl = path + "complaint/serial";
    ...
    //添加投诉
    const add = function () {
        const wid = Dialog.open("添加投诉信息", dialogWidth, dialogHeight, addUrl, [
    ...
    const edit = function () {
        ...
        const wid = Dialog.open("编辑投诉信息", dialogWidth, dialogHeight, editUrl + "?id=" + r.ID, [
            EasyUI.window.button("icon-save", "保存", function () {
    ...
    const dbClickEdit = function (index, row) {
        const wid = Dialog.open("编辑投诉信息", dialogWidth, dialogHeight, editUrl + "?id=" + row.ID, [
            EasyUI.window.button("icon-save", "保存", function () {
                EasyUI.form.submit("complaintForm", editUrl, function (data) {
                    filter();
                    Dialog.close(wid);
    ...
    function _common_product_onLoadSuccess() {
