...
<head>
    ...
    <script type="text/javascript">
        const onChange = function (newValue, oldValue) {
            const jtID = $("#ip").combobox('getValue');
            const url = path + "jt/viewComp?jtId=" + jtID;
            $.ajax({
                url: url,
                type: "get",
                dataType: "json",
...
<div id="toolbar" class="datagrid-toolbar">
    &nbsp; 监控机台屏幕&nbsp;
    <input id="ip" class="easyui-combobox" editable="false" prompt="请选择机台屏幕" url="${path}screen/combobox?all=1"
           data-options="
                  valueField: 'id',
                  panelMaxHeight:200,
                  textField: 'text',
...
<table id="deviceShow" class="tb_header" border="1" cellspacing="0" cellpadding="0" bordercolor="gray"
       style="BORDER-COLLAPSE: collapse">
    <tr>
        <th style="width:10%;">序号</th>
        <th style="width:35%;">客户产品
            <hr>
            厂内名称
        </th>
...
<script id="orders" type="text/template">
    ...
    <tr data-row="true" class="{{colorful(i)}} {{producing(value.ISPRODUCING)}} {{prior(value.SORT)}}">
        ...
        <td>{{code(value.PROCESSBOMCODE)}}({{value.PROCESSBOMVERSION}})
            <hr>
            {{code(value.PRODUCTPACKAGINGCODE)}}({{value.PRODUCTPACKAGEVERSION}})
        </td>
