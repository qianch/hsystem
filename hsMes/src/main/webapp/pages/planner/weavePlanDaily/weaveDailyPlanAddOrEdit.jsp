...
<div>
	<!--日计划表单-->
	<form style="height:98%" id="weaveDailyPlanForm" method="post" ajax="true" action="<%=basePath %>weaveDailyPlan/${empty weaveDailyPlan.id ?'add':'edit'}" autocomplete="off" >
		...
		<table width="100%">
				<tr>
					...
					<td>
						<input type="text"  id="planDate" name="planDate" value="${weaveDailyPlan.planDate}" ${empty weaveDailyPlan.planDate?"":"readonly"} class="easyui-datebox" required="true" >
					</td>
				
					<td class="title">车间:</td>
					<!--车间-->
					<td>
						<input type="text" id="workShop" name="workShop" value="${weaveDailyPlan.workShop}" ${empty weaveDailyPlan.workShop?"":"readonly"} class="easyui-combobox" required="true"
		...
		<div>
			<div id="dgg_add_toolbar">
				<a href="javascript:void(0)" class="easyui-linkbutton" plain="true" iconcls="icon-add" onclick="selectWeavePlan()">添加</a>
				<a href="javascript:void(0)" class="easyui-linkbutton" plain="true" iconcls="icon-edit" onclick="editWeavePlan()">编辑</a>
				<a href="javascript:void(0)" class="easyui-linkbutton" plain="true" iconcls="icon-remove" onclick="removeWeavePlan()">删除</a>
			</div>
		</div>
		<table id="dgg" toolbar="#dgg_add_toolbar" idField="ID" singleSelect="true" title="编织任务(双击行编辑产品属性、指定机台)"
