...
var weaveDailyPlan = path +"weaveDailyPlan/findW";
$(function() {
	var id = $("#ids").val();
	var workShop=$('#workShop').combobox('getValue');
	var nowTime=$('#planDate').datebox('getValue');
	...
	$('#dgg').datagrid({
						...
						onExpandRow : function(index, row) {
							...
							ddv.datagrid({
										url : path + "planner/weavePlan/device?wid="
												+ row.DAILYID+"&&workshop="+workShop+"&&date="+nowTime+"&&id="+row.ID,
										fitColumns : true,
										singleSelect : true,
										rownumbers : true,
										loadMsg : '',
...
<div>
	<form style="height:98%" id="weaveDailyPlanForm" method="post" ajax="true" action="<%=basePath %>weaveDailyPlan/${empty weaveDailyPlan.id ?'add':'edit'}" autocomplete="off" autoSearchFunction="false" >
		...
		<table width="100%">
				<tr>
					...
					<!--日计划计划时间-->
					<td>
						<input type="text" id="planDate" name="planDate" value="${weaveDailyPlan.planDate}" class="easyui-datebox" required="true" >
					</td>
				
					<td class="title">车间:</td>
					<!--车间-->
					<td>
						<input type="text" id="workShop" name="workShop" value="${weaveDailyPlan.workShop}" class="easyui-combobox" required="true"
							   data-options="valueField:'v',textField:'t',url:'<%=basePath %>department/queryDepartmentByType?type=weave'">
					</td>
				</tr>
		</table>
		<table id="dgg" singleSelect="false" title="编织计划列表"
			class="easyui-datagrid" url="" pagination="false"  rownumbers="true" fitColumns="true" fit="true">
			<thead>
				<tr>
