...
<script type="text/javascript">
    ...
    let _editingRow;
    const position =${empty details?'[]':position};
    ...
    function onChange(newValue, oldValue) {
        ...
        if (!isEmpty(text)) {
            const count = text.substring(text.indexOf("*") + 1).replace("托", "");
            const index = $(this).parent().parent().parent().parent().parent().parent().parent().attr("datagrid-row-index");
            const countEditor = $("#oldOrders").datagrid("getEditor", {
                index: index,
                field: 'TURNBAGCOUNT'
            });
    ...
    function comboboxLoadSuccess(data) {
        ...
        if (!isEmpty(text)) {
            ...
            const turnBagCount = $(countEditor.target).numberspinner("getValue");
            $(countEditor.target).numberspinner({
                max: parseInt(count)
            });
    ...
    function deleteOldSalesOrder() {
        const rows = $("#oldOrders").datagrid("getChecked");
        if (rows.length === 0) {
            Tip.warn("请至少选择一行");
            return;
        }
...
<div>
    <div>
        ...
        <table id="fbContent">
            <tr>
                ...
                <td><input type="text" style="border:none;background:white;" id="turnBagCode" value="${tbCode }"
                           readonly>
                </td>
                <td class="title">翻包部门</td>
                <td><input type="text" style="border:none;background:white;" id="turnBagDept" value="${tbDept }"
                           readonly>
                </td>
    ...
    <div id="toolbar_product">
        <a class="easyui-linkbutton" plain="true" iconCls="icon-add" onclick="addOldSalesOrder()">增加</a>
        <a class="easyui-linkbutton" plain="true" iconCls="icon-remove" onclick="deleteOldSalesOrder()">删除</a>
    </div>
    <table id="oldOrders" title="领料单" singleSelect="true" class="easyui-datagrid" fitColumns="false" fit="false"
        ...
        <tr>
            ...
            <th width="100"
                field="BATCHCODE" <%-- editor="{type:'combobox',options:{'required':true,'icons':[],onBeforeLoad:loadOldBatchCode,onLoadSuccess:comboboxLoadSuccess,url:'${path }planner/tbp/batchCode',valueField:'v',textField:'t'}}" --%>>
                翻包批次
            </th>
            <th width="100" field="TAKEOUTCOUNTFROMWAREHOUSE"
                editor="{type:'numberspinner',options:{min:0,max:9999,'required':true,width:'100%','icons':[],precision:0}}">
                仓库领出数量(托)
            </th>
            <th width="80" field="TURNBAGCOUNT"
                editor="{type:'numberspinner',options:{min:1,max:9999,'required':true,width:'100%','icons':[],precision:0}}">
                翻包数量(托)
            </th>
            <th width="150" field="MEMO" editor="{type:'textbox',options:{'required':false,width:'100%','icons':[]}}">
                备注
            </th>
            <th field="CONSUMERSIMPLENAME" width="100">客户简称</th>
