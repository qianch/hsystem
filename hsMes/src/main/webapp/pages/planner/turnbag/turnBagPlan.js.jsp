...
<script>
    //添加翻包计划
    const addUrl = path + "planner/tbp/add";
    //编辑翻包计划
    const editUrl = path + "planner/tbp/edit";
    //删除翻包计划
    const deleteUrl = path + "planner/tbp/delete";
    //打开提交审核页面
    const _auditCommitUrl = path + "selector/commitAudit";
    //提交审核流程
    const auditCommitUrl = path + "planner/tbp/commitAudit";
    //获取批次号
    const batchCode = path + "planner/tbp/batchCode";
    //获取明细
    const detailsUrl = path + "planner/tbp/details";
    const completeUrl = path + "planner/tbp/complete";
    const dialogWidth = 760, dialogHeight = 520;
    let wid, orderWid;
    ...
    function save() {
        const plan = JQ.getFormAsJson("turnBagPlanForm");
    ...
    function loadInfo(row) {
        ...
        //finishTime
        const cal = new Calendar(row.DELIVERYTIME);
    ...
    function onChange(newValue, oldValue) {
        ...
        if (!isEmpty(text)) {
            ...
            const countEditor = $("#oldOrders").datagrid("getEditor", {index: index, field: 'TURNBAGCOUNT'});
    ...
    const viewUrl = path + "audit/FB/{id}/state";
