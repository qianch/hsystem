	...
	<div data-options="region:'center'">
		<div id="weaveDgToolbar">
			<form id="dgForm" autoSearchFunction="false">
				车　　间:<input type="text" id="did" name="filter[did]" class="easyui-combobox" url="${path}device/scheduling/department" data-options="valueField:'ID',textField:'NAME',onChange:filter">
				机台号:<input type="text" id="dcodes" name="filter[dcodes]" in="true" class="easyui-textbox" prompt="A1,A2逗号分割">
				...
				订单交期：<input type="text" style="width:110px;" class="easyui-datebox" id="start" name="filter[start]" data-options="icons:[]">~<input type="text" style="width:110px;" class="easyui-datebox" id="end" name="filter[end]" data-options="icons:[]"><br>
		...
		<table id="dg" singleSelect="false"
			class="easyui-datagrid" toolbar="#weaveDgToolbar"  rownumbers="true" fitColumns="false" pagination="true" fit="true" data-options="rowStyler:isProducing,onBeforeLoad:beforeLoad,onLoadSuccess:loadSuccess">
			...
			<thead >
				<tr>
					...
					<th field="DRAWNO" sortable="true" width="150" data-options="editor:{type:'textbox',options:{required:false}}">图号</th>
					<th field="ROLLNO" sortable="true" width="80" data-options="editor:{type:'textbox',options:{required:false}}">卷号</th>
					<th field="LEVELNO" sortable="true" width="80" data-options="editor:{type:'textbox',options:{required:false}}">层数</th>
