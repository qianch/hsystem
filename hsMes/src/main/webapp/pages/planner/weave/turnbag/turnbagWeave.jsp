...
<div data-options="region:'center',border:false"
    ...
    <div id="toolbar">
        ...
        <a href="javascript:void(0)" plain="true" class="easyui-linkbutton" onclick="printBoxCode()" id="printBoxCode"
           iconCls="icon-print">打印盒条码</a>
        <a href="javascript:void(0)" plain="true" class="easyui-linkbutton" onclick="printTrayCode()" id="printTrayCode"
           iconCls="icon-print">打印托条码</a>
        <a href="javascript:void(0)" plain="true" class="easyui-linkbutton" onclick="viewTurnBagContent()"
           id="printTakeCountOrder" iconCls="icon-print">打印领料单</a>
        <div style="border-top:1px solid #DDDDDD">
            <form id="wpSearchForm" action="#" autoSearchFunction="false">
                车　　间:<input type="text" id="workShop" name="filter[workShopCode]" like="false" class="easyui-combobox"
                              prompt="请选择车间"
                              data-options="valueField:'v',textField:'t',url:'<%=basePath %>department/queryDepartmentByType?type=weave'">
                生产单号:<input type="text" class="easyui-textbox" prompt="生产计划单号" name="filter[code]" like="true"
                                data-options="onChange:filter">
                销售单号:<input type="text" class="easyui-textbox" prompt="销售单号" name="filter[saleCode]" like="true"
                                data-options="onChange:filter">
                批次号:<input type="text" class="easyui-textbox" prompt="批次号" name="filter[batchCode]" like="true"
                              data-options="onChange:filter"><br>
                规　　格:<input type="text" class="easyui-textbox" prompt="规格" name="filter[model]" like="true"
                              data-options="onChange:filter">
                米　　长:<input type="text" class="easyui-textbox" prompt="米长" name="filter[productLength]" like="true"
                              data-options="onChange:filter">
                门　　幅:<input type="text" class="easyui-textbox" prompt="门幅" name="filter[productWidth]" like="true"
                              data-options="onChange:filter"><br>
                完成状态:<input type="text" value="-1" name="filter[isFinish]" class="easyui-combobox"
                                prompt="已完成选项" data-options="onSelect:filter,onChange:filter,data: [
		                       {value:'3',text:'全部'},  {value:'-1',text:'未完成'}, {value:'1',text:'已完成'}]">
                关闭状态:<input type="text" value="0" name="filter[closed]" class="easyui-combobox"
                                prompt="已关闭选项" data-options="onSelect:filter,onChange:filter,data: [
		                       {value:'3',text:'全部'}, {value:'0',text:'未关闭'},{value:'1',text:'已关闭'}]">
                时　　间:<input type="text" id="start" name="filter[start]" value="" prompt="开始时间"
                              class="easyui-datetimebox" data-options="onChange:filter">
