...
<script>
    ...
    $(function () {
        const url = encodeURI("${path}planner/cut/turnbag/cutList?filter[isTurnBagPlan]=翻包");
        $('#dg').datagrid({
            url: url,
    ...
    function printBoxCode() {
        ...
        const wid1 = Dialog.open("托条码打印", 419, 200, path + "planner/produce/pack/turnbag", [EasyUI.window.button("icon-save", "打印", function () {
            const isValid = $("#tray-print-form").form('validate');
            if (!isValid) return;
            const tree = $("#printerTree").combotree("tree");
            const node = tree.tree('getSelected');
            if (tree.tree("getParent", node.target) == null) {
                Tip.warn("请选择打印机");
                return;
            }
            const c = $("#count").numberspinner("getValue");
            const weaveRow = $("#dg").datagrid("getSelected");
            /**
             * @param weavePlanId
             * @param cutPlanId
             * @param count
            ...
            Dialog.confirm(function () {
                JQ.ajaxPost(
                    ...
                    {
                        weavePlanId: weaveRow.ID,
                        count: c,
                        pName: node.id,
                        type: "box",
                        departmentName: weaveRow.WORKSHOPCODE
                    },
                    function (data) {
                        if (data.url) {
                            Tip.success("条码打印成功");
    ...
    function printTrayCode() {
        ...
        const wid1 = Dialog.open("部件条码打印", 419, 200, path + "planner/produce/pack/turnbag?fromSalesOrderDetailId="
            + r.FROMSALESORDERDETAILID + "&producePlanDetailId=" + r.PRODUCEPLANDETAILID
            + "&cutPlanId=" + r.ID + "&departmentName=" + workShopCode + "&trugPlanId=1", [EasyUI.window.button("icon-save", "打印", function () {
            const isValid = $("#tray-print-form").form('validate');
            if (!isValid) return;
            const c = $("#count").numberspinner("getValue");
            const weaveRow = $("#dg").datagrid("getSelected");
             ...
             */
            Dialog.confirm(function () {
                EasyUI.form.submit("doPrintBarcodeForm", path + "printer/doPrintBarcode", function (data) {
                    if (data.url) {
                        Tip.success("打印成功");
                        Dialog.close(dialogId);
                    } else {
