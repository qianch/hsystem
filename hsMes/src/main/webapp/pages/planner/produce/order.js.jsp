...
<script>
	...
	//修改完成状态
	var finish = path + "salesOrder/complete";
	var planed = path + "salesOrder/isPlaned";
	var dialogWidth = 700, dialogHeight = 500;
	var exportUrl = path + "salesOrder/export";
	var action = "add";
	var orderProduct= path + "planner/produce/order/list2";

	var currentConsumerCode = "";
	//获取要完成订单的详情
	var showProduce = path+"planner/produce/showProduce";
	var setAllocated = path+"planner/produce/allocated";
	//查询
	function filter() {
	...
	function reloadProduceOrder() {
		/* console.log(111);
		EasyUI.grid.search("orderProductSelect", "produceSalesOrderForm"); */
		var info = JQ.getFormAsJson("produceSalesOrderForm");
	...
	function dgLoadSuccess(){
		...
		/* var fs=$(this).datagrid("getColumnFields");
		var rules=[];
		for(var i=0;i<fs.length;i++){
			rules.push({field:fs[i],type:'textbox'});
		}
		
		fs=$(this).datagrid("getColumnFields",true);
		for(var i=0;i<fs.length;i++){
			rules.push({field:fs[i],type:'textbox'});
		}
		...
		$(".datagrid-filter[name='ISCOMPLETE']").remove();
		//$(this).datagrid("removeFilterRule", "ISCOMPLETE");
		$(this).datagrid("addFilterRule", {
			field:'ISCOMPLETE',
			type:'combobox',
			options:{data:[{"value":1,"text":"123"}]}
		}); */
		$("#orderProductSelect").datagrid("enableFilter",[
		                                                  {
		                                                	    "field": "CONSUMERNAME",
		                                                	  ...
		                                                	  },
		                                                	  {
		                                                	    ...
		                                                	    "options":{
		                                                	    	...
		                                                	    	valueField:"v",
		                                                	    	textField:"t",
		                                                	    	data:[{v:"",t:"全  部"},{v:"0",t:"未分配"},{v:"1",t:"部分已分配"},{v:"2",t:"全部已分配"}],
		                                                	  		onChange:function(nv,ov){
		                                                	  			$("#orderProductSelect").datagrid('addFilterRule', {
		                                                                    field: 'ISPLANED',
		                                                                    op: 'equal',
...
/* 	function orderRowClick(index, row) {
	...
	function completeOrder(){
		var r=EasyUI.grid.getSelections("orderProductSelect");
		if(r.length == 0){
			Tip.warn("<spring:message code="Tip.SelectAtLeastOne" />");
			return;
		}
