...
<script type="text/javascript">
	//JS代码
	var cutPlans = ${cutPlans};
	var weavePlans = ${weavePlans};
	$(document).ready(function(){
		...
		for(var a=0;a<cutPlans.length;a++){
			
			add_cutPlan_data(cutPlans[a]);
		}
		for(var a=0;a<weavePlans.length;a++){
...
<div>
	...
	<form id="producePlanForm" method="post" ajax="true"
		...
		<input type="hidden" name="id" value="${producePlan.id}" /> <input
			type="hidden" name="producePlanIsAutoCreate" value="2" /> <input
			type="hidden" name="auditState" value="${producePlan.auditState}" />
		<input type="hidden" name="producePlanDate"
			value="${producePlan.producePlanDate}" />

		<table width="100%">
			<tr>
				<td class="title">销售订单号:</td>
				<!--销售订单ID-->
				<td><input type="text" id="salesOrderCode"
					name="salesOrderCode" value="${producePlan.salesOrderCode}"
					class="easyui-textbox" readonly="true"></td>
			</tr>
			<tr>
				<td class="title">生产计划单号:</td>
				<!--生产计划单号-->
				<td><input type="text" id="producePlanCode"
			...
			<tr>
				...
				<td><input type="text" id="producePlanIsTemp"
					name="producePlanIsTemp" value="${producePlan.producePlanIsTemp==1?'非临时计划':'临时计划'}"
			...
			<tr>
				...
				<!--是否废弃  -1:废弃,其他状态为非废弃-->
				<td><input type="text" id="producePlanIsDeprecated"
					name="producePlanIsDeprecated"
					value="${producePlan.producePlanIsDeprecated==1?'非废弃':'废弃'}"
					class="easyui-textbox" readonly="true"
					>
				</td>
			</tr>
	...
	<div id="plan_detail" style="widht:100%;" class="easyui-layout" fit="true">
		<div id="weavePlan" class="easyui-layout" region="center" fit="true">
			<table id="dg_weavePlan" singleSelect="false" title="编织计划"
				class="easyui-datagrid" url="" rownumbers="true" fitColumns="true"
				fit="true">
				<thead>
					<tr>
						<th field="WEAVEPLANPRODUCTTYPE" sortable="true" width="15"
							formatter="formatterType">产品属性</th>
		...
		<div id="cutPlan" class="easyui-layout" region="east"
			style="width: 50%" >
			<table id="dg_cutPlan" singleSelect="true" title="裁剪计划"
				class="easyui-datagrid" url="" rownumbers="true" fitColumns="true"
				fit="true">
				<thead>
					<tr>
