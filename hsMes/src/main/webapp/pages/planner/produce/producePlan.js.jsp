...
<script>
	//添加生产任务单
	var addUrl = path + "planner/produce/add";
	//编辑生产任务单
	var editUrl = path + "planner/produce/edit";
	//删除生产任务单
	var deleteUrl = path + "planner/produce/delete";
	var exportUrl = path + "planner/produce/export";
	var exportUrl1 = path + "planner/produce/export1";
	var viewStockUrl = path + "stock/productStock/viewStock";

	//打开提交审核页面
	var _auditCommitUrl = path + "selector/commitAudit";

	var auditCommitUrl = path + "planner/produce/commitAudit";
	var add_req_detailUrl = path + "selector/addReq";
	//查看审核过的生产任务
	var look = path + "planner/produce/look";
	//关闭生产计划查看审核状态
	var closeU = path + "planner/produce/close";
	//关闭生产计划查看编制是否关闭
	var wclose = path + "planner/produce/wclose";
	var dialogWidth = 700, dialogHeight = 420;
	var _innerParam = null;
	var producePlanId = "";
	var pCode = null;
	var closeUrl = path + "common/close";
	var noclose = path + "planner/produce/noClose";
	var forceEditUrl=path + "planner/produce/forceEdit";
	...
	function doClose() {
		...
		var rows = $("#producePlanDetails").datagrid("getSelections");
	...
	function editReqDetails(v) {
		if(isEmpty(v)){
			...
			var i=$("#produceProducts").datagrid("getRowIndex", r);
			editingIndex=i;
			...
			var wid1 = Dialog.open("包装和工艺要求", dialogWidth, dialogHeight, add_req_detailUrl + "?pid=" + (isEmpty(r.id)?"-1":r.id) +"&productId="+r.productId, [ EasyUI.window.button("icon-save", "保存", function() {
				var packReq = $('#packReq').val();
				var procReq = $('#procReq').val();
				//updateRow
			...
			var r=$("#producePlanDetails").datagrid("getSelected");
			if(r==null)return;
			var wid1 = Dialog.open(r.PRODUCTMODEL+" 的包装和工艺要求", dialogWidth, dialogHeight, add_req_detailUrl + "?pid=-1&productId=-1", [EasyUI.window.button("icon-cancel", "关闭", function() {
				Dialog.close(wid1);
			}) ], function() {
				$("#" + wid1).dialog("maximize");
				$('#packReq').val(r.PACKREQ);
	...
	function packageTask() {
		var row=$("#producePlanDetails").datagrid("getSelected");
		if(row==null){
			Tip.warn("请选择生产计划");
			return;
		}
		...
		var wid1 = Dialog.open("包装和工艺要求", dialogWidth, dialogHeight,path+"planner/produce/pack/?producePlanDetailId="+row.ID+"&bcBomId="+row.PACKBOMID , [ EasyUI.window.button("icon-save", "保存", function() {
			...
			log(rows)
			for(var i=0;i<rows.length;i++){
				list.push({"producePlanDetailId":pid,"packageBomId":rows[i].PACKBOMID,"trayCount":rows[i].TRAYCOUNT});
			}
			$.ajax({
	...
	function saveForm() {
		...
		//-------------------------------------------------------------------------------------------------
		var workShopCode = $("#ws").combobox("getValue");
		var producePlanCode = $("#producePlanCode").textbox("getValue");
		var time = new Date().Format("yyyyMMdd");
		//sj-
		var sj = producePlanCode.substring(0, 3);
		//-bj1-
		var ws = producePlanCode.substring(11, 16);
		//计划单号时间
		var t = producePlanCode.substring(3, 11);
		...
		//当前时间
		var time = new Date().Format("yyyyMMdd");
		var year1 = time.substring(0, 4);
		var month1 = time.substring(4, 6);
		var day1 = time.substring(6, 8);
		var a = producePlanCode.split("-");
		...
		if (workShopCode == "00116") {
			...
			var ws1 = producePlanCode.substring(11,16);
			if (ws1 != "-CJ1-") {
				Tip.warn("单号格式不对");
				return;
			}
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (data.ret=="") {
					...
					var workShopCode = $("#ws").combobox("getValue");
					var list = $("#produceProducts").datagrid("getRows");
					var url = $("#producePlanForm").attr("action");
	...
	function orderProductSelect() {
		...
		selectSalesProductsId = Dialog.open("选择订单产品", 1000, 400, path + "/planner/produce/order/select?workShopCode="+$("#ws").combobox("getValue"), [ EasyUI.window.button("icon-save", "添加", function() {
		        	  ...
		        	  },
		        	  {
		        	    ...
		        	    "type": "textbox"
	                  	    ...
	                  	    "options":{
	                  	    	...
	                  	    	valueField:"v",
	                  	    	textField:"t",
	                  	    	data:[{v:"",t:"默认"},{v:"-1",t:"全部"},{v:"0",t:"未分配"},{v:"1",t:"部分已分配"},{v:"2",t:"全部已分配"}],
	...
	function onProcessSelect(record) {
		var ver = $("#produceProducts").datagrid('getEditor', {
			index : editingIndex,
			field : 'processBomVersion'
		});
		var code = $("#produceProducts").datagrid('getEditor', {
			index : editingIndex,
	...
	function processVersionLoaded() {
		...
		var items = $(ver.target).combobox("getData");
	...
	var doAudit = function() {
		...
		var wid = Dialog.open("审核", 480, 120, _auditCommitUrl + "?id=" + r.ID, [ EasyUI.window.button("icon-ok", "提交审核", function() {
	...
	function checkProcDetails() {
		...
		var wid = Dialog.open("生产物料需求", 900, 500, path + "selector/user?singleSelect=true", [ EasyUI.window.button("icon-cancel", "关闭", function() {
			Dialog.close(wid);
		}) ], function() {
			$("#" + wid).dialog("maximize");
		});
	...
	function turnBagContent(){
		...
		var r2=EasyUI.grid.getOnlyOneSelected("dg");
		debugger;
	...
	function _loadTask(){
		var row=EasyUI.grid.getOnlyOneSelected("producePlanDetails");
