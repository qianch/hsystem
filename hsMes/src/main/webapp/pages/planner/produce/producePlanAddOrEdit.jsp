...
<script type="text/javascript">
    ...
    function doOrderChange(nv,ov){
        ...
        var row=EasyUI.grid.getRowByIndex('produceProducts',index);
        if(!isEmpty(row)){
            if(row.productIsTc==1){
        ...
        //获取当前编辑行的行号
        var index=$(this).parent().parent().parent().parent().parent().parent().parent().attr("datagrid-row-index");
        var ddv = $("#produceProducts").datagrid('getRowDetail', index).find('table.ddv');
    ...
    function viewStock() {
        ...
        if (r == null)
            return;
        r.view=1;
        var wid = Dialog.open("厂内名称:[<font color=red>"+r.factoryProductName+"</font>]　　工艺代码:[<font color=red>"+r.processBomCode+"</font>]　　工艺版本:[<font color=red>"+r.processBomVersion+"</font>]",1500,650, viewStockUrl+ "?factoryProductName=" + r.factoryProductName+"&"+"productProcessCode=" + r.processBomCode+"&"+"productProcessBomVersion=" + r.processBomVersion+"&"+"workShopCode=" + w, [ EasyUI.window.button("icon-cancel", "关闭", function() {
            Dialog.close(wid);
        }) ], function() {
            $("#" + wid).dialog("maximize");
        });
...
<div style="height:98%;width:98%">
        ...
        <table width="100%">
            <tr>
                ...
                <td><input type="text" id="ws" name="workShopCode"
                           value="${producePlan.workShopCode}"
                ${empty producePlan.workShopCode?"":"readonly"} class="easyui-combobox"
                           required="true"
                           data-options="valueField:'v',textField:'t',url:'<%=basePath %>department/queryDepartmentByType?type=cut,weave',
                                onSelect:onSelect,icons:[]"
                          >
                </td>
                <td class="title">生产任务单号:</td>
                <!--生产任务单号-->
                <td><input type="text" editable="true" id="producePlanCode"
                <c:if test="${!empty force }"> readonly='true' </c:if>
                           name="producePlanCode" value="${producePlan.producePlanCode}"
                           class="easyui-textbox" required="true" data-options="icons:[]">
                </td>
                <td class="title">创建人:</td>
                <!--创建人-->
                <td><input type="text" editable="true" id="producePlanCode" readonly
                           name="producePlanCode" value="${userName}"
                           class="easyui-textbox" required="true" data-options="icons:[]">
                </td>
            </tr>
        </table>
    ...
    <div id="produceSalesOrderToolbar">
        <c:if test="${empty force }">
            <a href="javascript:void(0)" class="easyui-linkbutton" plain="true"
               iconcls="icon-add" onclick="orderProductSelect()">添加产品</a>
            <a href="javascript:void(0)" class="easyui-linkbutton" plain="true"
               iconcls="icon-remove" onclick="orderProductRemove()">删除产品</a>
        </c:if>
        <a href="javascript:void(0)" class="easyui-linkbutton" plain="true"
           iconcls="icon-edit" onclick="editReqDetails()">修改 工艺/包装 需求</a>
        <a href="#" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-tip'" onClick="viewStock()">查看库存</a>
    </div>
    <table id="produceProducts" toolbar="#produceSalesOrderToolbar"
        ...
        <tr>
            ...
            <th field="isTurnBagPlan" width="60"
                editor="{type:'combobox',options:{'valueField':'v','textField':'t','required':true,'icons':[],'data':[{'v':'生产','t':'生产'},{'v':'翻包','t':'翻包'}]}}">计划类型</th>
            <th field="salesOrderCode" width="100">订单号</th>
            <th field="salesOrderSubcodePrint" width="100" hidden="true">客户订单号</th>
            <th field="consumerName" width="200">客户名称</th>
            <th field="batchCode" width="90"
                    <c:if test="${empty force }"> editor="{type:'textbox',options:{required:true}}" </c:if>>批次号</th>
            <th field="orderCount" data-options="formatter:countFormatter">订单数量</th>
            <th width="80" field="orderTotalWeight"
                formatter="totalWeightFormatter2">总重量</th>
            <th width="80" field="orderTotalMetres"
                formatter="totalMetresFormatter">总米长</th>
            <th field="requirementCount" width="100"
                data-options="formatter:planCountFormatter,editor:{type:'numberbox',options:{required:true,precision:0,min:0,'icons':[],onChange:doOrderChange} }">排产数量</th>
            ...
            <th width="80" field="planAssistCount"
                editor="{type:'numberbox',options:{'required':false,'icons':[],min:0,precision:2}}">排产辅助数量</th>
            <th width="80" field="planAssistUnit"
                editor="{type:'combobox',options:{'required':false,'icons':[],valueField:'v',textField:'t',data:[{'v':'','t':'　'},{'v':'KG','t':'KG'},{'v':'套','t':'套'}]}}">计划辅助单位</th>
            ...
            <th field="totalTrayCount" width="100"
                data-options="formatter:totalTrayCountFormatter,editor:{type:'numberbox',options:{'required':false,precision:0,min:1,'icons':[],suffix:'托'}}">总托数</th>
            <th field="deleveryDate" width="100"
                data-options="editor:{type:'datebox',options:{required:true,editable:false}}">出货日期</th>
            ...
            <th field="deviceCode" width="100"
                data-options="editor:{type:'textbox'}">机台号</th>
    ...
    <table style="width:100%;height:35%;table-layout: fixed;">
        ...
        <tr style="height: 90%;">
            <td style="padding: 5px;"><pre id="addOrEditPackReq"
                                           name="packReq" style="width: 100%; height:100%"></pre></td>
            <td style="padding: 5px;"><pre id="addOrEditProcReq"
                                           name="procReq" style="width: 100%; height:100%"></pre></td>
        </tr>
    </table>
</div>
