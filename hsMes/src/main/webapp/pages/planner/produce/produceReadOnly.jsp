...
<div style="height:98%">
	...
	<form id="producePlanForm" method="post" ajax="true" action="<%=basePath %>planner/produce/${empty producePlan.id ?'add':'edit'}" autocomplete="off" >
		...
		<table width="100%">
				<tr>
					...
					<!--生产任务单号-->
					<td>
						<input type="text" id="workshop" name="workshop" value="${producePlan.workshop}" ${empty producePlan.workshop?"":"readonly"} class="easyui-combobox" required="true" readonly="true"
						data-options="data: [
					...
					<!--生产任务单号-->
					<td>
						<input type="text" editable="true" id="producePlanCode" name="producePlanCode" value="${producePlan.producePlanCode}"  class="easyui-textbox" required="true" readonly="true" data-options="icons:[]" >
					</td>
				</tr>
		</table>
	</form>
	<table  id="produceProducts" 
			...
			<thead>
				<tr>
					<th field="productWidth" width="80" data-options="editor:{type:'numberbox',options:{precision:4,min:1,'icons':[]}}" >门幅(mm)</th>
					<th field="productWeight" width="80" data-options="editor:{type:'numberbox',options:{precision:4,min:1,'icons':[]}}">卷重(kg)</th>
					<th field="productLength" width="80" data-options="editor:{type:'numberbox',options:{precision:4,min:1,'icons':[]}}">卷长(m)</th>
					...
					<th field="requirementCount" width="110" data-options="editor:{type:'numberbox',options:{required:true,precision:4,min:1,'icons':[]}}" >生产数量(kg/套)</th>
					<th field="totalRollCount" width="80" data-options="editor:{type:'numberbox',options:{precision:0,min:1,'icons':[]}}" >总卷数</th>
					<th field="totalTrayCount" width="80" data-options="editor:{type:'numberbox',options:{precision:0,min:1,'icons':[]}}" >总托数</th>
					<th field="deleveryDate" width="100"  data-options="editor:{type:'datebox',options:{required:true,editable:false}}">出货日期</th>
