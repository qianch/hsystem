...
<div class="easyui-layout" style="height:100%;width:100%;">
	<div data-options="region:'north',height:112">
		...
		<form id="salesOrderForm" style="padding:0px;margin:0px;">
			...
			<input type="hidden" id="fromProducePlancode" name="fromProducePlancode" value="${empty fromProducePlancode?salesOrder.fromProducePlancode:fromProducePlancode}" >
		   ...
		   <table width="100%">
				<tr>
					...
					<td>
						...
						<input type="text" id="salesOrderConsumerName"  value="${consumerName}" class="easyui-searchbox" required="true" ></td>
					
					<td class="title">内销/外销:</td>
					<!--内销/外销-->
					<td><input type="text" id="salesOrderIsExport" name="salesOrderIsExport" value="${salesOrder.salesOrderIsExport}" class="easyui-combobox"  data-options="valueField:'v',textField:'t',data:[{'v':'-1','t':'胚布'}],onSelect:changeSerial"></td>
				...
				<tr>
					...
					<!--下单日期-->
					<td><input type="text" id="salesOrderDate" name="salesOrderDate" value="${salesOrder.salesOrderDate}" class="easyui-datebox" required="true" readonly="true"></td>
				...
				<tr>
					...
					<!--业务员-->
					<td>
						<input type="hidden" name="salesOrderBizUserId" id="salesOrderBizUserId" value="${empty salesOrder.salesOrderBizUserId?userId:salesOrder.salesOrderBizUserId }">
					 <input type="text" id="salesOrderBizUserName" value="${empty bizUserName?userName:bizUserName}" class="easyui-searchbox" required="true"
						></td>
					<td class="title">订单类型:</td>
					<!--订单类型 （3新品，2试样，1常规产品，-1未知）-->
					<td><input type="text" id="salesOrderType" required="true" name="salesOrderType" value="${salesOrder.salesOrderType}" class="easyui-combobox" data-options="valueField:'v',textField:'t',data:[{'v':'1','t':'常规'},{'v':'2','t':'试样'},{'v':'3','t':'新品'},{'v':'-1','t':'未知'}],value:1"></td>
				</tr>
				<tr>
					<td class="title">订单备注:</td>
					<!--订单备注-->
					<td colspan="3">
						<textarea style="width:100%;height:25px;border-radius:5px;" name="salesOrderMemo" maxlength="2000">${salesOrder.salesOrderMemo}</textarea>
					</td>
				</tr>
			</table>
		</form>
	...
	<div data-options="region:'center'">
		<div id="toolbar_product">
			...
			<input type="text" class="easyui-datebox" id="sss"><a href="javascript:void(0)" class="easyui-linkbutton" plain="true" iconcls="icon-edit" onclick="resetDate()">批量修改时间</a>
			<input type="text" class="easyui-textbox" id="ssss"><a href="javascript:void(0)" class="easyui-linkbutton" plain="true" iconcls="icon-edit" onclick="resetSales()">批量修改订单号</a>
		</div>
		<table id="product_dg" title="订单产品" fit="true" class="easyui-datagrid" singleSelect="false" fitColumns="false" style="width:100%;" toolbar="#toolbar_product" data-options="onClickRow:clickRow">
			<thead frozen="true">
				<tr>
					<th field="productId" checkbox=true></th>
					<th width="120" field="salesOrderSubCode" editor="{type:'textbox',options:{required:true,'icons':[]}}">订单号</th>
					<th width="120" field="productBatchCode" editor="{type:'textbox',options:{required:false,'icons':[]}}">批次号</th>
					...
					<th width="80" field="productCount" editor="{type:'numberbox',options:{'required':true,'icons':[]}}" formatter="processNumberFormatter">数量(kg/套)</th>
