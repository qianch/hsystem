...
/* 		var url=path+"finishProduct/list";
	...
	$(function() {
		var url = "<%=basePath %>finishProduct/list?filter[productIsTc]=-1&filter[auditState]=2&filter[consumerId]=${finalConsumerId}";
	...
	function _commons_product_dg_onBeforeLoad_callback() {
		var firstLoad = $("#_common_product_dg").attr("firstLoad");
...
<div id="_common_product_toolbar">
	<div style="border-top:1px solid #DDDDDD">
		...
		<!-- 	客户代码：<input type="text" name="filter[consumerCode]" like="true" class="easyui-textbox">
			...
			产品型号：<input type="text" id="productModel" name="filter[productModel]" like="true" class="easyui-textbox">
			显示作废：<input type="text" name="filter[old]"  class="easyui-combobox" data-options="data: [{value:'1',text:'已作废'}]">
			<a href="javascript:void(0)" class="easyui-linkbutton" iconcls="icon-add" onclick="copyProduct()">复制</a><br>
			门　　幅：<input type="text" id="productWidth" name="filter[productWidth]" like="true" class="easyui-textbox">
			工艺代码：<input type="text" id="procCode" name="filter[productProcessCode]" like="true" class="easyui-textbox">
			工艺版本：<input type="text" id="procVersion" name="filter[productProcessBomVersion]" like="true" class="easyui-textbox" >
...
<table id="_common_product_dg"  width="100%" height="98%"  idField="ID" data-options="pageSize:100,nowrap:false,rowStyler:productStyler" singleSelect="true" class="easyui-datagrid"  toolbar="#_common_product_toolbar" pagination="true" rownumbers="true" fitColumns="false" fit="true">
	<thead frozen="true">
		<tr>
