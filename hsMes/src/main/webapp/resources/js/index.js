...
var Assert=Assert=Assert||{};Assert.isEmpty=function(a){if(a==null||a==undefined){return true}if(!a.replace(new RegExp(" ","gm"),"").replace(new RegExp("	","gm"),"")){return true}return false};Assert.isNumber=function(a){if(+a===a){return true}if(typeof a=="string"){if(a.indexOf("-")==0){a=a.replace("-","")}return a.match(/\d*/i)==a}return false};Assert.isFloat=function(a){if(+a===a){return true}if(typeof a=="string"){if(a.indexOf("-")==0){a=a.replace("-","")}return/^\d+(\.\d+)?$/.test(a)}return false};Assert.isArray=function(a){return a&&a.constructor==Array};Assert.isTypeOf=function(b,a){return(typeof b==a)};var EasyUI=EasyUI=EasyUI||{};EasyUI.grid={search:function(a,b){$("#"+a).datagrid("reload",JQ.getFormAsJson(b))},search2:function(a,b){$("#"+a).datagrid("reload",filter)},getSelections:function(a){return $("#"+a).datagrid("getSelections")},getOnlyOneSelected:function(a){var b=EasyUI.grid.getSelections(a);if(b.length>1){Tip.warn(tipSelectOnlyOne);return null}if(b.length==0){Tip.warn(tipSelectAtLeastOne);return null}return b[0]},contains:function(b,d){var c=$("#"+b).datagrid("getData");if(c.total==0){return false}else{for(var a=0;a<c.rows.length;a++){if(c.rows[a]["ID"]==d.ID){return true}}return false}},contains2:function(c,e,a){var d=$("#"+c).datagrid("getData");if(d.total==0){return false}else{for(var b=0;b<d.rows.length;b++){if(d.rows[b][a]==e[a]){return true}}return false}},size:function(a){var b=$("#"+a).datagrid("getData");return b.total},getRowByIndex:function(b,c){var a=$("#"+b).datagrid("getData");for(var d=0;d<a.rows.length;d++){if(d==c){return a.rows[d]}}},getRowIndex:function(a,b){return $("#"+a).datagrid("getRowIndex",b)},removeRow:function(a,b){$("#"+a).datagrid("deleteRow",b)},appendRow:function(a,b){if(!EasyUI.grid.contains(a,b)){$("#"+a).datagrid("appendRow",b)}},empty:function(a){var c=$("#"+a).datagrid("getRows");for(var b=c.length-1;b>=0;b--){$("#"+a).datagrid("deleteRow",$("#"+a).datagrid("getRowIndex",c[b]))}}};EasyUI.form={valid:function(b){var a=$("#"+b).form("validate");return a},submit:function(d,b,c,a){if(!EasyUI.form.valid(d)){return}Loading.show(tipSubmiting);$.ajax({url:b,type:"post",data:$("#"+d).serialize(),dataType:"json",success:function(e){Loading.hide();if(!Utils.isNull(c)&&c instanceof Function){c(e)}},error:function(e){Loading.hide();if(!Utils.isNull(a)&&a instanceof Function){a(e)}}})},clear:function(a){$("#"+a).form("clear")}};EasyUI.window={button:function(d,e,c){var a={};a.iconCls=d==undefined?null:d;a.text=e==undefined?"&nbsp;":e;a.handler=c==undefined?null:c;return a},open:function(h,d,a,c,e,g,b){var f=new Date().getTime();$("body").append("<div id='"+f+"'></div>");$("#"+f).dialog({id:f,title:h,width:d,height:a,href:c,modal:true,maximizable:true,minimizable:false,resizable:true,buttons:e,onClose:function(){$("#"+f).dialog("destroy");$("#"+f).remove();if(typeof b==="function"){b()}},onLoad:function(){if(typeof g==="function"){g()}}});return f},close:function(a){$("#"+a).dialog("destroy");$("#"+a).remove()}};EasyUI.combobox=function(d,b,c,a){};EasyUI.datebox={formatter:function(a){return a.format("yyyy-MM-dd")}};EasyUI.datetimebox={formatter:function(a){return a.format("yyyy-MM-dd HH:mm:ss")}};EasyUI.tree={getTreeData:function(b,d,a,c){function g(k){var j=[];for(var h=0;h<b.length;h++){if(b[h][c]==k.id){j.push({id:b[h][d],text:b[h][a]})}}for(var h=0;h<j.length;h++){j[h].children=g(j[h])}return j}var f=[];for(var e=0;e<b.length;e++){if(b[e][c]==null){f.push({id:b[e][d],text:b[e][a]})}}for(var e=0;e<f.length;e++){f[e].children=g(f[e])}return f},select:function(b,c){var a=$("#"+b).tree("find",c);$("#"+b).tree("expandTo",a.target).tree("select",a.target)}};function auditStyler(a,b){return""}function auditStyler2(a,b){return""}function auditStateFormatter(a){if(a==1){return'<span style="background:#dedddd;border: 1px solid #a9a4a4;color:#FF8C00;font-weight:bold;" class="l-btn-left l-btn-icon-left"><span class="l-btn-text">审核中</span><span class="l-btn-icon icon-question">&nbsp;</span></span>'}else{if(a==2){return'<span style="background:#dedddd;border: 1px solid #a9a4a4;color:green;font-weight:bold;" class="l-btn-left l-btn-icon-left"><span class="l-btn-text">已通过</span><span class="l-btn-icon icon-ok3">&nbsp;</span></span>'}else{if(a==0){return'<span style="background:#dedddd;border: 1px solid #a9a4a4;color:#5a080f;font-weight:bold;" class="l-btn-left l-btn-icon-left"><span class="l-btn-text">未提交</span><span class="l-btn-icon icon-commit">&nbsp;</span></span>'}else{if(a==-1){return'<span style="background:#dedddd;border: 1px solid #a9a4a4;color:#FF0000;font-weight:bold;" class="l-btn-left l-btn-icon-left"><span class="l-btn-text">不通过</span><span class="l-btn-icon icon-del">&nbsp;</span></span>'}}}}}function autoAuditStateFormatter(b,c,a){if(b==1){return'<span style="background:#dedddd;border: 1px solid #a9a4a4;color:#FF8C00;font-weight:bold;" class="l-btn-left l-btn-icon-left"><span class="l-btn-text">审核中</span><span class="l-btn-icon icon-question">&nbsp;</span></span>'}else{if(b==2){return'<span style="background:#dedddd;border: 1px solid #a9a4a4;color:green;font-weight:bold;" class="l-btn-left l-btn-icon-left"><span class="l-btn-text">已通过</span><span class="l-btn-icon icon-ok3">&nbsp;</span></span>'}else{if(b==0){return'<span style="background:#dedddd;border: 1px solid #a9a4a4;color:#5a080f;font-weight:bold;" class="l-btn-left l-btn-icon-left"><span class="l-btn-text">未提交</span><span class="l-btn-icon icon-commit">&nbsp;</span></span>'}else{if(b==-1){return'<span style="background:#dedddd;border: 1px solid #a9a4a4;color:#FF0000;font-weight:bold;" class="l-btn-left l-btn-icon-left"><span class="l-btn-text">不通过</span><span class="l-btn-icon icon-del">&nbsp;</span></span>'}}}}}function bomAuditStateFormatter(a){if(a==1){return'<span style="color:#FF8C00;font-weight:bold;">[审核中]</span>'}else{if(a==2){return'<span style="color:green;font-weight:bold;">[已通过]</span>'}else{if(a==0){return'<span style="color:#5a080f;font-weight:bold;">[未提交]</span>'}else{if(a==-1){return'<span style="color:#FF0000;font-weight:bold;">[不通过]</span>'}}}}}function auditTreeStyler(a){if(a.attributes.nodeType=="version"){return a.text+bomAuditStateFormatter(a.attributes.AUDITSTATE)}else{return a.text}}var JQ=JQ=JQ||{};JQ.getFormAsJson=function(h){var e="{";var a=$("#"+h).serializeArray();var b=$("#"+h+" input[like=true]");var c=$("#"+h+" input[in=true]");var f=new Array();var d=new Array();$.each(b,function(i,j){f.push(Utils.isEmpty($(j).attr("textboxname"))?$(j).attr("name"):$(j).attr("textboxname"))});$.each(c,function(i,j){d.push(Utils.isEmpty($(j).attr("textboxname"))?$(j).attr("name"):$(j).attr("textboxname"))});$.each(a,function(i,j){if(Utils.isEmpty(j.value)){return}if(f.contains(j.name)){e=e+(e=="{"?"":",")+'"'+j.name+'":"like:'+j.value+'"'}else{if(!d.contains(j.name)){e=e+(e=="{"?"":",")+'"'+j.name+'":"'+j.value+'"'}}});var g="";$.each(d,function(i,j){g="";$.each(a,function(l,k){if(j==k.name){if(k.value==""){return}g=g+(g==""?"":",")+k.value}});if(g==""){return}e=e+(e=="{"?"":",")+'"'+j+'":"in:'+g+'"'});e=e+"}";e=e.replace(/\r\n/g,"\\r\\n");e=e.replace(/\n/g,"\\r\\n");return JSON.parse(e)};JQ.getFormAsString=function(h){var g="";var a=$("#"+h).serializeArray();var b=$("#"+h+" input[like=true]");var c=$("#"+h+" input[in=true]");var e=new Array();var d=new Array();$.each(b,function(i,j){e.push(Utils.isEmpty($(j).attr("textboxname"))?$(j).attr("name"):$(j).attr("textboxname"))});$.each(c,function(i,j){d.push(Utils.isEmpty($(j).attr("textboxname"))?$(j).attr("name"):$(j).attr("textboxname"))});$.each(a,function(i,j){if(Utils.isEmpty(j.value)){return}if(e.contains(j.name)){g=g+(g==""?"":"&")+j.name+"=like:"+j.value}else{if(!d.contains(j.name)){g=g+(g==""?"":"&")+j.name+"="+j.value}}});var f="";$.each(d,function(i,j){f="";$.each(a,function(l,k){if(k.value==""){return}if(j==k.name){f=f+(f==""?"":",")+k.value}});if(f==""){return}g=g+(g==""?"":"&")+j+"=in:"+f});return g};JQ.ajax=function(d,c,e,a,b){Loading.show();$.ajax({url:d,type:c,dataType:"json",data:e==undefined?null:e,success:function(f,g){Loading.hide();if(a==undefined||a==null){return}a(f)},error:function(g,i,h){Loading.hide();if(b==undefined||b==null){return}var f=JSON.parse(g.responseText);b(f)}})};JQ.ajaxGet=function(c,a,b){JQ.ajax(c,"GET",null,a,b)};JQ.ajaxPost=function(c,d,a,b){JQ.ajax(c,"POST",d,a,b)};JQ.setValue=function(a,b){$(a).val(b)};JQ.getValue=function(a){return $(a).val()};JQ.setAttr=function(b,a,c){$(b).attr(a,c)};JQ.getAttr=function(b,a){return $(b).attr(a)};JQ.setCss=function(b,a,c){$(b).css(a,c)};JQ.getCss=function(b,a){return $(b).css(a)};function notify(b,a){Tip.success(a);if(!("Notification" in window)){}else{if(Notification.permission==="granted"){show(b,a)}else{if(Notification.permission!=="denied"){Notification.requestPermission(function(c){if(c==="granted"){show(b,a)}})}}}}function show(c,b){var c=c;var a={body:b,icon:"resources/notification/notice.png"};if(document.getElementById("_notice_bg_sound")==null){$("body").append('<audio id="_notice_bg_sound" src="resources/notification/notice.mp3" ></audio>')}document.getElementById("_notice_bg_sound").play();var d=new Notification(c,a);d.onclick=function(){window.focus();d.close()}}var Dialog=Dialog=Dialog||{};Dialog.open=function(h,d,a,c,e,g,b){var f=EasyUI.window.open(h,d,a,c,e,g,b);return f};Dialog.more=function(a){$("#"+a).parent().find("div.dialog-button").prepend('<div style="float:left;font-size:12px;"><input style="vertical-align:middle;" type="checkbox" id="addMore"><label style="vertical-align:middle;" for="addMore">'+buttonAddMore+"</label></div>")};Dialog.isMore=function(a){if($("#"+a).parent().find(".dialog-button").find("input[type=checkbox]").attr("checked")=="checked"){return true}return false};Dialog.max=function(a){$("#"+a).dialog("maximize")};Dialog.min=function(a){$("#"+a).dialog("minimize")};Dialog.close=function(a){EasyUI.window.close(a)};Dialog.confirm=function(a,b){$.messager.confirm(tipInfo,Utils.isEmpty(b)?tipConfirm:b,function(c){if(c){a()}})};var Loading=Loading=Loading||{};Loading.show=function(a){$("<div id='_uiloading' class=\"window-mask\" style='z-index:9998'></div>").css({display:"block",width:"100%",height:$(window).height()}).appendTo("body");$("<div id='_uiloading_text' class=\"datagrid-mask-msg\" style='z-index:9999;height:10px;line-height:10px;font-size:10px;'></div>").html(a==undefined?"请稍等(Loading)...":a).appendTo("body").css({display:"block",left:($(document.body).outerWidth(true)-190)/2,top:($(window).height()-45)/2})};Loading.hide=function(){$("#_uiloading").remove();$("#_uiloading_text").remove()};var Tip=Tip=Tip||{};Tip.warn=function(a){$.messager.show({title:tipInfo,msg:"<div class='messager-icon messager-warning'></div>"+a,showType:"show"})};Tip.success=function(a){$.messager.show({title:tipInfo,msg:"<div class='messager-icon messager-info'></div>"+a,showType:"show"})};Tip.error=function(a){$.messager.show({title:tipInfo,msg:"<div class='messager-icon messager-error'></div>"+a,showType:"show"})};Tip.hasError=function(a){if(a==null||a==undefined||a.error==undefined){return false}return true};Tip.dealError=function(b){if(b.readyState==0){Tip.error("服务器已关闭或者服务器地址请求不到");return}var a=JSON.parse(b.responseText);Tip.error(a.error)};
var menus;
$(function() {
	initNotice();
	var a = 0
...
function addTab(title,iconCls, url) {
	if ($('#tt').tabs('exists', title)) {
		...
		$('#tt').tabs('add', {
			...
			closable : true,
			tools:[{
				iconCls:'platform-max',
				handler:function(){
					var txt = $(this).parent().parent().find(".tabs-title").text();
					var src = $("#tt").tabs("getTab", txt).panel("body").find("iframe").attr("src");
					window.open(src,txt);
				}
			},{
				...
				handler:function(){
					var txt = $(this).parent().parent().find(".tabs-title").text();
					var src = $("#tt").tabs("getTab", txt).panel("body").find("iframe").attr("src");
					$("#tt").tabs("getTab", txt).panel("body").find("iframe")[0].src = src;
				}
			}]
		});
	}
