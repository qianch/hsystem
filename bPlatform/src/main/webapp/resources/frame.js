...
function InitTopMenu() {
	$.ajax({
		...
		url : "menu/mymenu",// 与此页面沟通
		// url:"platform/bcmenu.json",
		dataType : 'json',// 返回的值以 JSON方式 解释
		timeout : 8 * 1000,
		success : function(json) {
			InitsubMenu(json[0].CHILDREN);// 初始化左边的菜单
		}
	});
}
	...
	$.each(data, function(i, sm) {
		...
		if (sm.CHILDREN) {
			$.each(sm.CHILDREN, function(k, o) {// 二级菜单
				menulist += '<li class="subtitle"><div><a ref="' + o.ID + '" href="javascript:void(0)" rel="' + o.URL + '" ><span class="icon-icon ' + o.ICON + '" >&nbsp;</span><span  class="nav">' + o.TEXT + '</span</a></div>';
				if (o.CHILDREN) {
					submenu += '<ul>'
					$.each(o.CHILDREN, function(m, n) {// 三级菜单
						submenu += '<li><div><a ref="' + n.ID + '" href="javascript:void(0)" rel="' + n.URL + '" ><span class="icon-icon ' + n.ICON + '" >&nbsp;</span><span class="nav">' + n.TEXT + '</span></a></div></li>'
					});
					submenu += '</ul>'
			...
			menulist += '<li><div><a ref="' + sm.ID + '" href="javascript:void(0)" rel="' + sm.URL + '" ><span class="icon-icon' + sm.ICON + '" >&nbsp;</span><span class="nav">' + sm.TEXT + '</span></a></div></li> ';
		}
		menulist += '</ul>';
		$("#wnav").accordion('add', {
			title : sm.TEXT,
			content : menulist,
			iconCls : 'icon-icon ' + sm.ICON
		});
	});
...
function addTab(subtitle, url, icon, menuid) {
	...
	$.ajax({
		 ...
		 * if(textStatus=="timeout"){ $.messager.alert('加载超时','对不起，加载用户信息超时！<br>请刷新页面',
		 * "info"); $("#rightmask").hide() $("#progressBar_right").hide();
		 * 
		 * }else{ // $.messager.alert("加载失败", "对不起，加载用户信息失败！<br>请刷新页面",
		 * "error"); // $("#rightmask").hide() //
		 * $("#progressBar_right").hide(); window.location.href =
		 * MES_BASE+"/login.jsp";
		 *  } },
		 */
		beforeSend : function() {
