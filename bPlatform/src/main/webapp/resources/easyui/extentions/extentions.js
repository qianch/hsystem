...
(function($){
	$.extend($.fn.datagrid.defaults, {
		onHeaderContextMenu: function(e,field){
			...
			var col = $('#'+_current_grid_context_id_).datagrid('getColumnOption', fields[0]);
			var item=$('#_grid_header_menu_').menu("findItem",col.title);
			//显示datagrid的列
			if(item==null){
				for(var i=0; i<fields.length; i++){
	                var field = fields[i];
...
function fixFrozenColumns(){
	...
	if(frozenCols.length!=0){
		$("#"+_current_grid_context_id_).datagrid("unfreezeColumn",frozenCols[0]);
		$("#"+_current_grid_context_id_).datagrid("freezeColumn",frozenCols[0]);
	}else{
		$("#"+_current_grid_context_id_).datagrid("freezeColumn",cols[0]);
		$("#"+_current_grid_context_id_).datagrid("unfreezeColumn",cols[0]);
	}
