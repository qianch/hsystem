...
function chunkFormatString(formatStr) {
	var chunks = [];
	var chunker = /\[([^\]]*)\]|\(([^\)]*)\)|(LTS|LT|(\w)\4*o?)|([^\w\[\(]+)/g; //  more descrimination
	var match;
...
var DragListener = FC.DragListener = Class.extend(ListenerMixin, MouseIgnorerMixin, {
	...
	//  do this for wrappers that have overflow:hidden as well.
	scrollEl: null,
...
var Grid = FC.Grid = Class.extend(ListenerMixin, MouseIgnorerMixin, {
	...
	minResizeDuration: null, //  hack. set by subclasses. minumum event resize duration

	// if defined, holds the unit identified (ex: "year" or "month") that determines the level of granularity
	// of the date areas. if not defined, assumes to be day and time granularity.
	//  port isTimeScale into same system?
	largeUnit: null,
	...
	--------------------------------------------------------------------------------------------------------------------
	 remove this system. like we did in TimeGrid
	*/
...
Grid.mixin({
	...
	segs: null, // the *event* segments currently rendered in the grid.  rename to `eventSegs`
	...
	eventSpanToSegs: function(span, event, segSliceFunc) {
		...
		for (i = 0; i < segs.length; i++) {
			seg = segs[i];
			seg.event = event;
			seg.eventStartMS = +span.start; //  not the best name after making spans unzoned
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	sliceRangeByTimes: function(range) {
		...
		for (dayIndex = 0; dayIndex < this.daysPerRow; dayIndex++) {
			dayDate = this.dayDates[dayIndex].clone(); //  better API for this?
			dayRange = {
...
TimeGrid.mixin({
	...
	renderBgSegs: function(segs) {
		segs = this.renderFillSegEls('bgEvent', segs); //  old fill system
	...
	renderHighlightSegs: function(segs) {
		segs = this.renderFillSegEls('highlight', segs); //  old fill system
		this.updateSegVerticals(segs);
	...
	renderBusinessSegs: function(segs) {
		segs = this.renderFillSegEls('businessHours', segs); //  old fill system
		this.updateSegVerticals(segs);
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	clearEvents: function() {
		...
		if (this.isEventsRendered) {
			...
			//  optimize: if we know this is part of a displayEvents call, don't queryScroll/setScroll
			scrollState = this.queryScroll();

			this.triggerEventUnrender();
			if (this.destroyEvents) {
				this.destroyEvents(); //  deprecate
			}
			this.unrenderEvents();
			this.setScroll(scrollState);
...
var Calendar = FC.Calendar = Class.extend({
	...
	buildViewSpec: function(requestedViewType) {
		...
		while (viewType) {
			...
			if (typeof spec === 'function') { //  deprecate
				spec = { 'class': spec };
			}
...
Calendar.defaults = {
	...
	buttonText: {
		...
		prevYear: "prev year",
		nextYear: "next year",
		year: 'year', //  locale files need to specify this
		today: 'today',
		month: 'month',
		week: 'week',
		day: 'day'
...
Calendar.rtlDefaults = { // right-to-left defaults
	header: { //  smarter solution (first/center/last ?)
...
//  document the structure and ordering of a FullCalendar locale file
...
function EventManager() { // assumed to be a calendar
	...
	function normalizeEventTimes(eventProps) {
		...
		if (eventProps.allDay) {
			...
			if (eventProps.end) {
				//  consider nextDayThreshold here? If so, will require a lot of testing and adjustment
