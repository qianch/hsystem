...
<head>
    ...
    <script type="text/javascript" src="<%=site%>resources/ui/js/js.v2.js"></script>
    ...
    <script type="text/javascript">
        ...
        let nodes;
        $(document).ready(function () {
            $('#tree_filter').keyup(function (e) {
                if (e.keyCode == 13) {
                    filterNodes = [];
                    treeFilter(nodes);
                    initTree(filterNodes);
                }
            });
            JQ.ajaxPost(path + 'menu/mymenu', {}, function (data) {
                nodes = data.children;
                initTree(nodes);
            });
            $("#locale").val(locale);
            ...
            $('#mt').tree({
                ...
                onClick: function (node) {
                    if (node.attributes.url) {
                        addTab(node.text, node.iconCls, node.attributes.url);
                    } else {
                    ...
                    nodes = data;
                    $('#mt').tree("collapseAll");
                    $("#mt").tree("expand", $('#mt').tree("getRoots")[0].target);
                }
            });
        }

        function treeFilter(nodes) {
            const filterVal = $.trim($("#tree_filter").val());
            if (!filterVal) {
                filterNodes = nodes;
                return;
            }
            ...
            for (let i = 0; i < nodes.length; i++) {
                if (reg.test(nodes[i].text)) {
                    let node = jQuery.extend({}, nodes[i]);
                    node.text = node.text.replace(reg, '<b style="color: blue;">$&</b>');
                    node.children = [];
                    filterNodes.push(node);
                }
                if (nodes[i].children.length != 0) {
                    treeFilter(nodes[i].children);
                }
            }
        }

        function revert() {
            const $filterTree = $("#tree").next("ul.filterTree");
...
<div id="leftTitle" data-options="region:'west',split:true,title:'功能菜单'" style="width:200px;">
    <input id="tree_filter" class='easyui-searchbox' style="width:85%;position:fixed!important;left:5px;top:30px;"
           data-options="
                    prompt: '功能查询',
                    icons:[{
                        	iconCls:'icon-clear',
